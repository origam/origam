	...
	public override void Run()
	{
		switch(this.MethodName)
		{
			case "SendMail":
				...
				_result = MailServiceFactory.GetMailService().SendMail(this.Parameters["Data"] as IXmlContainer, server, port);
				
				break;
			case "RetrieveMails":
				// Check input parameters
				if(! (this.Parameters["UserName"] is string))
					throw new InvalidCastException(ResourceUtils.GetString("ErrorMailServerNotString"));
				if(! (this.Parameters["Password"] is string))
					throw new InvalidCastException(ResourceUtils.GetString("ErrorPasswordNotString"));
				if(! (this.Parameters["Server"] is string))
					throw new InvalidCastException(ResourceUtils.GetString("ErrorServerNotString"));
				if(! (this.Parameters["Port"] is int))
					throw new InvalidCastException(ResourceUtils.GetString("ErrorPortNotInt"));
				_result = AbstractMailService.GetMails(this.Parameters["Server"] as String, Convert.ToInt32(this.Parameters["Port"]), this.Parameters["UserName"] as String, this.Parameters["Password"] as String, this.TransactionId, 0);
