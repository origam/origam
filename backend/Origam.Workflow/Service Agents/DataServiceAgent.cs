	...
	public class DataServiceAgent : AbstractServiceAgent
	{
		...
		private DataSet LoadData(DataStructureQuery query, DataSet data)
		{
			...
			if (method == null || method is DataStructureFilterSet)
			{
				if (data == null)
				{
					return _dataService.LoadDataSet(query, SecurityManager.CurrentPrincipal, this.TransactionId);
				}
				else
				{
					return _dataService.LoadDataSet(query, SecurityManager.CurrentPrincipal, data, this.TransactionId);	
				}
			}
			else if (method is DataStructureWorkflowMethod)
			{
				...
				{
					throw new System.InvalidCastException(String.Format("{0} (Try to change "
			...
			else
			{
				throw new ArgumentException(String.Format(
					"MethodId ({0}) schema item element not found or not of proper type", method.Id));
			}
		}
		...
		private object GetScalarValue(DataStructureQuery query, ColumnsInfo columnsInfo)
		{
			...
			else if (method is DataStructureWorkflowMethod)
			{
				...
				{
					// return contextstore not found
					throw new ArgumentException(String.Format("Return context store not found for workflow {0}.",
						dataStructureWorkflowMethod.LoadWorkflowId));
				}
				else if (!returnContext.isScalar())
				{
					// return contextstore is not scalar
					throw new ArgumentException(String.Format("Return context store is not of scalar type for workflow {0}",
						dataStructureWorkflowMethod.LoadWorkflowId));
				}
				// call workflow
		...
		private IDataDocument ExecuteProcedure(string name, Hashtable parameters, string entityOrder)
		{
			...
			DataSet result = _dataService.ExecuteProcedure(name, entityOrder, query, this.TransactionId);
			if (result == null)
			{
				return null;
			}
            ...
            IDataDocument data, bool forceBulkInsert)
		{
			...
			try
			{
				_dataService.UpdateData(
                    query, SecurityManager.CurrentPrincipal, dataset, 
            ...
            DataStructureQuery query, DataSet data)
		{
			try
			{
                ...
                _dataService.UpdateData(
                    query, principal, data, TransactionId);
			}
			catch (ConstraintException)
			{
				// make the exception far more verbose
		...
		private int UpdateReferences(Guid entityId, object oldValue, object newValue)
		{
			...
			if(this.Trace)
			{
				trace.TraceStep(this.TraceWorkflowId, this.TraceStepName, this.TraceStepId, "Deduplication", "Start", originalEntity.MappedObjectName, "", "", "Old Id: " + oldValue.ToString() + Environment.NewLine + "New Id: " + newValue.ToString());
			}
			...
			try
			{
				foreach(IDataEntity entity in entities.ChildItems)
				{
					...
					if(table != null && table.DatabaseObjectType == DatabaseMappingObjectType.Table && table.GenerateDeploymentScript)
					{
						...
						{
							if(column.ForeignKeyEntity != null && column.ForeignKeyEntity.PrimaryKey.Equals(originalEntity.PrimaryKey) && column is FieldMappingItem)
							{
								if(column.ForeignKeyField != null && column.ForeignKeyField.PrimaryKey.Equals(originalKey.PrimaryKey))
								{
									int records = this.DataService.UpdateField(table.Id, (Guid)column.PrimaryKey["Id"], oldValue, newValue, SecurityManager.CurrentPrincipal, transactionId);
									result += records;
		...
		private int ReferenceCount(Guid entityId, object value)
		{
			...
			if(this.Trace)
			{
				trace.TraceStep(this.TraceWorkflowId, this.TraceStepName, this.TraceStepId,  "ReferenceCount", "", originalEntity.MappedObjectName, "", "", "Id: " + value.ToString());
			}
					...
					if(table != null && table.DatabaseObjectType == DatabaseMappingObjectType.Table && table.GenerateDeploymentScript)
					{
						foreach(IDataEntityColumn column in table.EntityColumns)
						{
							if(column.ForeignKeyEntity != null && column.ForeignKeyEntity.PrimaryKey.Equals(originalEntity.PrimaryKey) && column is FieldMappingItem)
							{
								if(column.ForeignKeyField != null && column.ForeignKeyField.PrimaryKey.Equals(originalKey.PrimaryKey))
								{
									int records = this.DataService.ReferenceCount(table.Id, (Guid)column.PrimaryKey["Id"], value, SecurityManager.CurrentPrincipal, transactionId);
									result += records;
								}
							}
						}
			...
			switch(this.MethodName)
			{
				...
				case "StoreData":
					...
					if(dsRef == null)
					{
						...
						{
							throw new InvalidCastException(ResourceUtils.GetString("ErrorNotDataStructureReference"));
						}
				...
				case "LoadDataByQuery":
					if(this.Parameters.Contains("Data"))
					{
						...
						_result = this.LoadData(this.Parameters["Query"] as DataStructureQuery, this.Parameters["Data"] as DataSet);
					}
					else
					{
				...
				case "StoreDataByQuery":
					...
					_result = this.SaveData(this.Parameters["Query"] as DataStructureQuery,
				...
				case "ExecuteProcedure":
					...
					_result = this.ExecuteProcedure((string)this.Parameters["Name"]
						, this.Parameters["Parameters"] as Hashtable
						, entityOrder);
					break;
				...
				case "UpdateReferences":
					...
					_result = this.UpdateReferences((Guid)this.Parameters["EntityId"],
				...
				case "ReferenceCount":
					...
					_result = this.ReferenceCount((Guid)this.Parameters["EntityId"],
                ...
                case "ExecuteSql":
                    _result = DataService.ExecuteUpdate((string)Parameters["Command"], this.TransactionId);
                    break;
			...
			if(method == "LoadData" && parameter == "Parameters")
			{
				...
				if(ds != null)
				{
					...
					if(wm != null)
					{
						...
						return agent.ExpectedParameterNames(wm.LoadWorkflow, "ExecuteWorkflow", "Parameters");
					}
					else
					{
