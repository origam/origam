		...
		protected override void OnExecute()
		{
			...
			if (updateTask.OutputContextStore.Structure != null)
			{
				...
				if (res is IDataDocument) 
				{
					...
					RuleEngine.ConvertStringValueToContextValue(origamDataType, val, ref valueToContext);
                    if (valueToContext == null) valueToContext = DBNull.Value;
				...
				else
				{
					...
					throw new WorkflowException(ResourceUtils.GetString("ErrorWrongContextStoreForUpdate", updateTask.OutputContextStore.Name));
				}
			}
			else
			{
            ...
            if (Engine.IsTrace(Step))
            {
				tracingService.TraceStep(
					...
					"Result",
					updateTask.OutputContextStore.Name,
					changed ? WorkflowEngine.ContextData(this.Engine.RuleEngine.GetContext(outputCtxtKey)) : "-- no change --",
					null,
					null);
			}
