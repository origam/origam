...
public class WorkQueueFileLoader : WorkQueueLoaderAdapter
{
    ...
    )
    {
        ...
        if (_readType == ReadType.SplitByRows)
        {
            ...
            {
                throw new Exception(ResourceUtils.GetString("ErrorSplitFileByRows"));
            }
            if (_mode == FileType.BINARY)
            {
                throw new Exception(ResourceUtils.GetString("SplitBinaryFilesNotSupported"));
            }
        }
