			...
			else
			{
				try
				{
					...
					lock(profileCacheByIdentity)
					{
						...
						DataRow obpRow = null;
						if (result.Tables["OrganizationBusinessPartner"].Rows.Count > 0
							&& CURRENT_ORGANIZATION_BUSINESS_PARTNER_ID__NOT_SET_YET.Equals(switchBusinessPartnerId))
						{
							obpRow = result.Tables["OrganizationBusinessPartner"].Rows[0];
						}

						// switched to some particular organization business partner?
						if (switchBusinessPartnerId != null
				...
				catch(Exception ex)
				{
					...
					throw new Exception(
						...
						+ Environment.NewLine 
						+ ResourceUtils.GetString("ErrorUnableToLoadProfile2"), ex);
				}
			}
		}
	#endregion
