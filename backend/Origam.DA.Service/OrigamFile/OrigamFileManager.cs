...
public class OrigamFileManager : IDisposable
{
    ...
    public void WriteReferenceFileToDisc(
        string fullPath,
        string contents,
    ...
    {
        string xmlToWrite = OrigamDocumentSorter.CopyAndSort(xmlDocument).ToBeautifulString();
        fileEventQueue.Pause();
    ...
    public void RemoveDirectoryIfEmpty(DirectoryInfo oldFullDirectory)
    {
        ...
        bool isEmpty = !oldFullDirectory.GetAllFilesInSubDirectories().Any();
        if (isEmpty)
        {
