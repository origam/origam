...
namespace Origam.DA.Service
{
    public class OrigamXDocument
    {
        ...
        {
            string currentValue = XDocument
                .Root?.Attributes(XName.Get(nameSpaceName, "http://www.w3.org/2000/xmlns/"))
                .FirstOrDefault()
                ?.Value;
        ...
        public void RenameNamespace(XNamespace oldNamespace, XNamespace updatedNamespace)
        {
            ...
            FileElement.Add(
                new XAttribute(
                    XNamespace.Xmlns + namespaceAttribute.Name.LocalName,
                    updatedNamespace
                )
            );
            ...
            foreach (XElement el in FileElement.Descendants())
            {
                ...
                el.Attributes()
                    ...
                    .ForEach(attr =>
                    {
                        ...
                        el.Add(
                            new XAttribute(
                                updatedNamespace.GetName(attr.Name.LocalName),
                                attr.Value
                            )
                        );
                    });
        ...
        {
            var nodeNamespaces = FileElement.Descendants().Select(node => node.Name.NamespaceName);
            ...
            FileElement.Attributes().Where(attr => !usedNamespaces.Contains(attr.Value)).Remove();
        }
        ...
        public XNamespace FindClassNamespace(string fullTypeName)
        {
            return FileElement
                ...
                ?? throw new Exception(
                    $"Could not find namespace for class {fullTypeName} in {XDocument}"
                );
        }
    }
}
