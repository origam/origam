...
public class TrackerSerializationData
{
    ...
    [ProtoMember(3)]
    private readonly AutoIncrementedIntIndex<Guid> guidIndex = new AutoIncrementedIntIndex<Guid>();
    ...
    public IEnumerable<TypeInfo> PersistedTypeInfos => typeIndex.IdToValue.Values;
    private IDictionary<string, int> CategoryIdDictionary => categoryIndex.ValueToId;

    private IDictionary<int, string> IdCategoryDictionary => categoryIndex.IdToValue;

    private TrackerSerializationData() { }

    public TrackerSerializationData(
        IEnumerable<ITrackeableFile> origamFiles,
    ...
    private void ToSerializationForms(
        IEnumerable<ITrackeableFile> origamFiles,
        IDictionary<string, int> categoryIdDictionary
    )
    {
        serializationList = origamFiles
            .Select(orFile => new OrigamFileSerializedForm(orFile, this))
            .ToList();
    }
    ...
    private List<ITrackeableFile> TransformBack(OrigamFileFactory origamFileFactory)
    {
        ...
        return serializationList
            .Select(serForm => serForm.GetOrigamFile(this, origamFileFactory))
            .ToList();
    }
    ...
    public override string ToString()
    {
        ...
        return "TrackerSerializationdata:\n"
            + "ElementIdDictionary: "
            + CategoryIdDictionary.Print()
            + spacer
            + "serializationList: ["
            + serializationList.Select(x => x.ToString()).Aggregate("", (x, y) => $"{x}\n{y}")
            + "]\n"
            + spacer
            + "guidIndex: "
            + guidIndex
    ...
    private class OrigamFileSerializedForm
    {
        ...
        [ProtoMember(4)]
        private IDictionary<int, int> ParentFolderIdsNums { get; } = new Dictionary<int, int>();

        private OrigamFileSerializedForm() { }

        public OrigamFileSerializedForm(
        ...
        )
        {
            ...
            ContainedObjInfos = origamFile
                .ContainedObjects.Values.Select(objInfo => new ObjectInfoSerializedForm(
                    objInfo,
                    trackerData
                ))
                .ToList();
            ...
            ParentFolderIdsNums = origamFile.ParentFolderIds.ToDictionary(
        ...
        )
        {
            ITrackeableFile trackableFile = origamFileFactory.New(
                ...
                fileHash: FileHash,
                parentFolderIds: ParentFolderIdsNums.ToDictionary(
        ...
        )
        {
            foreach (ObjectInfoSerializedForm objInfoSf in ContainedObjInfos)
            {
                ...
                PersistedObjectInfo objInfo = objInfoSf.GetObjectInfo(origamFile, trackerData);
    ...
    private class ObjectInfoSerializedForm
    {
        ...
        public ObjectInfoSerializedForm(
        ...
        )
        {
            ...
            TypeId = trackerData.typeIndex.AddValueAndGetId(
                new TypeInfo(objInfo.FullTypeName, objInfo.Version)
            );
        }

        public PersistedObjectInfo GetObjectInfo(
            OrigamFile origamFile,
            TrackerSerializationData trackerData
        )
        {
            return new PersistedObjectInfo(
