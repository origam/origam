...
public abstract class AbstractDataService : IDataService
{
	...
	internal DataStructureEntity GetDataStructureEntity(DataStructureQuery query)
	{
		...
		try
		{
			result = this.PersistenceProvider.RetrieveInstance(typeof(DataStructureEntity), 
                new ModelElementKey(query.DataSourceId)) as DataStructureEntity;
		}
		catch
		{
			throw new ArgumentOutOfRangeException("id", query.DataSourceId, ResourceUtils.GetString("DataStructureNotFound"));
		}
		if(result == null)
		{
			throw new ArgumentOutOfRangeException("id", query.DataSourceId, ResourceUtils.GetString("DataStructureNotFound"));
	...
	internal DataStructure GetDataStructure(Guid id)
	{
		...
		if(result == null)
		{
			throw new ArgumentException(ResourceUtils.GetString("StructureNotInModel", id.ToString()));
		}
		return result;
	}
	...
	internal FieldMappingItem GetTableColumn(Guid id)
	{
		...
		if(result == null)
		{
			throw new ArgumentException(ResourceUtils.GetString("FieldMappingNotInModel", id.ToString()));
		}
		return result;
	}
	internal DataStructureFilterSet GetFilterSet(Guid id)
	{
		...
		try
		{
			retrievedInstance = this.PersistenceProvider.RetrieveInstance(
                typeof(DataStructureFilterSet), new ModelElementKey(id));
		}
		catch
		{
			throw new ArgumentOutOfRangeException(
                "id", id, ResourceUtils.GetString("FilterSetNotFound"));
		}
	    DataStructureFilterSet result 
            = retrievedInstance as DataStructureFilterSet;
        if (result == null)
        {
            throw new ArgumentException(ResourceUtils.GetString(
                "InvalidTypeRetrieved", id, "DataStructureFilterSet", 
	...
	internal DataStructureSortSet GetSortSet(Guid id)
	{
		...
		try
		{
			result = this.PersistenceProvider.RetrieveInstance(typeof(DataStructureSortSet), 
                new ModelElementKey(id)) as DataStructureSortSet;
		}
		catch
		{
			throw new ArgumentOutOfRangeException("id", id, ResourceUtils.GetString("SortSetNotFound"));
		}
		return result;
	}
	internal DataStructureDefaultSet GetDefaultSet(Guid id)
	{
		...
		try
		{
			result = this.PersistenceProvider.RetrieveInstance(typeof(DataStructureDefaultSet), 
                new ModelElementKey(id)) as DataStructureDefaultSet;
		}
		catch
		{
			throw new ArgumentOutOfRangeException("id", id, ResourceUtils.GetString("DefaultSetNotFound"));
		}
		return result;
	}
				...
				if(param != null)
				{
					...
					try
					{
						...
						else
						{
							...
							{
								case DbType.Guid:
									if(param.Value is Guid)					value = param.Value;
									else if(param.Value is string & dbParam.IsNullable && ((string)param.Value == "") ) value = DBNull.Value;
									else if(param.Value is String)			value = new Guid(param.Value.ToString());
									else if(param.Value == DBNull.Value)	value = DBNull.Value;
									break;
								case DbType.DateTime:
									...
									{
										value = DateTime.Now.AddDays(Convert.ToDouble((int)param.Value));
									}
									else
									{
								...
								case DbType.Currency:
									...
									{
										value = System.Xml.XmlConvert.ToDecimal(param.Value as string);
									}
									else
									{
										value = param.Value;
					...
					{
						throw new Exception(ResourceUtils.GetString("ErrorWhenSettingParam", 
                            dbParam.ParameterName, param.Value), ex);
					}
				}
			}
		}
        ...
        int overrideActionType)
	{
		...
		if((EntityAuditingType)table.ExtendedProperties[
            Const.EntityAuditingAttribute] 
		...
		foreach(DataRow row in table.Rows)
		{
			foreach(DataColumn column in table.Columns)
			{
				if(!column.ExtendedProperties.Contains(
                    Const.TemporaryColumnAttribute)
                && !column.ExtendedProperties.Contains(Const.ArrayRelation)
                && ((EntityAuditingType)column.ExtendedProperties[
                    Const.EntityAuditingAttribute] 
                != EntityAuditingType.None)
                    ...
                    row[column, DataRowVersion.Original]))))
                {
                    ...
                    {
                        logRow.RecordCreatedBy 
                            = (Guid)row["RecordUpdatedBy"];
                    }
                    else
                    {
                        if(profile != null)
                        ...
                        Const.DefaultLookupIdAttribute))
                    {
                        ...
                        Guid lookupId = (Guid)column.ExtendedProperties[
                            Const.DefaultLookupIdAttribute];
                        if(row.RowState != DataRowState.Deleted)
                        {
                        ...
                        if(row.RowState != DataRowState.Added)
                        {
                            logRow.OldValue = lookupService.GetDisplayText(lookupId, 
                                row[column, DataRowVersion.Original], 
                                transactionId).ToString();
                            logRow.OldValueId = row[column, 
                    ...
                    else
                    {
                        ...
                        if(row.RowState != DataRowState.Deleted)
                        {
                            logRow.NewValue = row[column, 
                                DataRowVersion.Current].ToString();
                        }
                        if(row.RowState != DataRowState.Added)
                        {
                            logRow.OldValue = row[column, 
                                DataRowVersion.Original].ToString();
                        }
                    }
	...
	public DataSet GetEmptyDataSet(Guid dataStructureId, CultureInfo culture)
	{
		...
		DataStructure ds = _persistence.RetrieveInstance(typeof(DataStructure), key) as DataStructure;
