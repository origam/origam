...
{
    public override string FullTypeName { get; } = typeof(Schema.WorkflowModel.Workflow).FullName;
    ...
    public WorkflowScriptContainer()
    {
        upgradeScripts.Add(
            new UpgradeScript(
                new Version("6.0.0"),
                new Version("6.0.1"),
                (node, doc) => RemoveAttribute(node, "traceLevel")
            )
        );
        upgradeScripts.Add(
            new UpgradeScript(
                new Version("6.0.1"),
                new Version("6.0.2"),
                (node, doc) => AddAttribute(node, "onFailure", "WorkflowFails")
            )
        );
    }
}
