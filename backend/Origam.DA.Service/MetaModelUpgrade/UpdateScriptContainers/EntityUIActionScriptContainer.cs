...
class EntityUIActionScriptContainer : UpgradeScriptContainer
{
    ...
    public override string[] OldPropertyXmlNames { get; } = { "screen", "screenSection" };

    public EntityUIActionScriptContainer()
    {
        upgradeScripts.Add(
            new UpgradeScript(new Version("6.0.0"), new Version("6.1.0"), UpgradeTo610())
        );
    ...
    private Action<XElement, OrigamXDocument> UpgradeTo610()
    {
        return (node, doc) =>
        {
            ...
            var screenAttribute = node.Attributes()
            ...
            if (screenAttribute != null)
            {
                ...
                screenConditionNode.SetAttributeValue(
                    persistenceNamespace.GetName("id"),
                    Guid.NewGuid().ToString()
                );
                screenConditionNode.SetAttributeValue(
                    asiNamespace.GetName("name"),
                    "ScreenSectionCondition1"
                );
                screenConditionNode.SetAttributeValue(srNamespace.GetName("screen"), screenValue);
                node.Add(screenConditionNode);
            }
            var screenSectionAttribute = node.Attributes()
            ...
            if (screenSectionAttribute != null)
            {
                ...
                XNamespace ssrNamespace =
                    "http://schemas.origam.com/Origam.Schema.GuiModel.ScreenSectionCondition/6.0.0";
                doc.AddNamespace(
                    "ssc",
                    "http://schemas.origam.com/Origam.Schema.GuiModel.ScreenSectionCondition/6.0.0"
                );
                XElement screenSectionConditionNode = new XElement(
                    ssrNamespace.GetName("ScreenSectionCondition")
                );
                screenSectionConditionNode.SetAttributeValue(
                    persistenceNamespace.GetName("id"),
                    Guid.NewGuid().ToString()
                );
                screenSectionConditionNode.SetAttributeValue(
                ...
                screenSectionConditionNode.SetAttributeValue(
                    ssrNamespace.GetName("screenSection"),
                    screenSectionAttributeValue
                );
