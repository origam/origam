...
public abstract class UpgradeScriptContainer
{
        ...
        Version fromVersion, Version toVersion)
    {
            ...
            if (scriptsToRun[0].FromVersion != fromVersion)
            {
                ...
                {
                    // The first version used to be 6.0.0, the script container was probably created with that assumption.      return Upgrade(documentContainer, classNode, ClassMetaVersionAttribute.FormerFirstVersion, toVersion);
                }
                else
                {
