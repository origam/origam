...
public class PgSqlCommandGenerator : AbstractSqlCommandGenerator
{
    ...
    public override void DeriveStoredProcedureParameters(IDbCommand command)
    {
        ...
        command
            .Parameters.OfType<NpgsqlParameter>()
    ...
    public override string CreateOutputTableSql(string tmpTable)
    {
        return string.Format(
            "CREATE TEMP TABLE {0} ON COMMIT DROP AS {1}",
    ...
    public override string FunctionDefinitionDdl(Function function)
    {
        if (function.FunctionType == OrigamFunctionType.Database)
        {
            ...
            builder.Append("RETURN " + sqlRenderer.ParameterReferenceChar + Environment.NewLine);
    ...
    private NpgsqlDbType ConvertDataType(OrigamDataType columnType, DatabaseDataType dbDataType)
    {
        if (dbDataType != null)
        {
            return (NpgsqlDbType)
                Enum.Parse(typeof(NpgsqlDbType), dbDataType.MappedDatabaseTypeName);
        }
        switch (columnType)
        {
