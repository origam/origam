...
public class DatasetGenerator
{
    ...
    public bool UserDefinedParameters
    {
        ...
        set { _userDefinedParameters = value; }
    }
     ...
     * returns null.
     * */
    private static object EvaluateParameters(
    ...
    )
    {
        ...
        ICustomParameter customParameter = CustomParameterService.MatchParameter((string)param);
    ...
    private void AddTableFromEntity(IDataEntity entity, string name, DataSet dataset)
    {
        ...
        foreach (IDataEntityColumn column in entity.EntityColumns)
        {
            if (!column.ExcludeFromAllFields)
            {
                ...
                AddColumnExtendedProperties(tableColumn, column, entity, column.DefaultLookup);
                ...
                if (detachedField != null && detachedField.DataType == OrigamDataType.Array)
                {
                    ...
                    tableColumn.ExtendedProperties.Add(
                        ...
                        detachedField.ArrayValueField.Name
                    );
    ...
    private DataSet CreateDataSet(
        ...
        DataStructureDefaultSet defaultSet,
        CultureInfo culture,
        bool forceBuildFromDatastructure
    )
    {
        if (ds == null)
        {
        ...
        string cacheKey =
            ds.Id.ToString()
            + (defaultSet == null ? Guid.Empty.ToString() : defaultSet.Id.ToString());
        ...
        if (ds.DataSetClass != null && ds.DataSetClass.Trim() != "" && !forceBuildFromDatastructure)
        {
            ...
            DataSet outDS =
                Reflector.InvokeObject(
                    splittedDataSetClass[0].Trim(),
                    splittedDataSetClass[1].Trim()
                ) as DataSet;
            ...
            DataSet generatedDS = CreateDataSet(
                ds,
        ...
        DataSet dataset = new DataSet(dataSetName);
        StateMachineSchemaItemProvider states =
            (
        ...
        foreach (DataStructureEntity entity in entities)
        {
            ...
            if (
                !(entity.Entity is IAssociation && (entity.Entity as IAssociation).IsSelfJoin)
            ...
            )
            {
                ...
                {
                    if (
                        entity.EntityDefinition.Caption == null
                        || entity.EntityDefinition.Caption == ""
                    )
                    {
                    ...
                    {
                        table.DisplayExpression = DatasetTools.TextExpression(
                            entity.EntityDefinition.Caption
                        );
                    }
                }
                ...
                foreach (DataStructureColumn column in entity.Columns)
                {
                    ...
                    switch (column.Field)
                    {
                        ...
                        {
                            var relatedEntity = LookupRelation(entity, detachedField.ArrayRelation);
                    ...
                    tableColumn.ExtendedProperties.Add(
                        "OnCopyAction",
                        finalColumn.Field.OnCopyAction
                    );
                    // auditing exclusion
                    ...
                    if (states != null)
                    {
                        sm = states.GetMachine(
                            (Guid)entity.EntityDefinition.PrimaryKey["Id"],
                            (Guid)finalColumn.Field.PrimaryKey["Id"]
                        );
                    }
                    if (sm == null)
                    {
                    ...
                    if (column.UseCopiedValue)
                    {
                        AddExtendedProperties(
                            tableColumn.ExtendedProperties,
                            new ModelElementKey(Guid.Empty)
                        );
                        tableColumn.ExtendedProperties.Add(
                            Const.OriginalFieldId,
                            column.Field.PrimaryKey["Id"]
                        );
                    ...
                    else if (column.UseLookupValue)
                    {
                        ...
                        tableColumn.ExtendedProperties.Add(
                            Const.OriginalLookupIdAttribute,
                            column.FinalLookup.PrimaryKey["Id"]
                        );
                        ...
                        else
                        {
                            ...
                            tableColumn.ExtendedProperties.Add(
                                Const.IsAddittionalFieldAttribute,
                                true
                            );
                        }
                    }
                    else
                    {
                        ...
                        if (
                            column.Entity == null
                            && functionCall != null
                            && functionCall.Function.FunctionType == OrigamFunctionType.Standard
                            && functionCall.ForceDatabaseCalculation == false
                            && column.Aggregation == AggregationType.None
                        )
                        {
                            isCalculated = true;
                        ...
                        if (column.Field is AggregatedColumn || column.IsFromParentEntity())
                        {
                            isCalculated = true;
                            if (includeCalculatedColumns)
                            {
                    ...
                    if (
                        includeCalculatedColumns
                        || (includeCalculatedColumns == false && isCalculated == false)
                    )
                    {
                        table.Columns.Add(tableColumn);
                    }
                    ...
                    if (
                        ...
                        && column.IsWriteOnly == false
                        && column.Entity == null
                        && column.Aggregation == AggregationType.None
                    )
                    {
                        bool columnInPkAlready = DatasetTools.IsAliasedColumn(
                            table.PrimaryKey,
                            (Guid)column.Field.PrimaryKey["Id"]
                        );
                ...
                foreach (DataStructureColumn column in nonPhysicalColumns)
                {
                    try
                    {
                        table.Columns[column.Name].Expression = RenderExpression(
                            column.Field,
                            entity
                        );
                    }
                    catch (Exception e)
                    {
                        throw new OrigamException(
                            string.Format(
                                "Error occured while rendering column `{0}', ({1}) - {2}",
                                column,
                                column.Id,
                                e.Message
                            ),
        ...
        foreach (DataStructureColumn column in aggregatedColumns)
        {
            ...
            {
                dataset.Tables[entity.Name].Columns[column.Name].Expression = RenderParentField(
                    column
                );
            }
            else
            {
                dataset.Tables[entity.Name].Columns[column.Name].Expression = RenderExpression(
                    column.Field,
                    entity
                );
            }
    ...
    private void SetDefault(
        DataStructureDefaultSet defaultSet,
        DataStructureColumn finalColumn,
    ...
    )
    {
        ...
        {
            foreach (
                var d in defaultSet.ChildItemsByType<DataStructureDefaultSetDefault>(
                    DataStructureDefaultSetDefault.CategoryConst
                )
            )
            {
                if (
                    d.Field.PrimaryKey.Equals(finalColumn.Field.PrimaryKey)
                    & d.Entity.PrimaryKey.Equals(entity.PrimaryKey)
                )
                {
                    if (d.Default != null)
                    {
    ...
    private string GetCaption(IDataEntityColumn column)
    {
        ...
        // label
        var dynamicLabels = column.ChildItemsByType<EntityFieldDynamicLabel>(
            EntityFieldDynamicLabel.CategoryConst
        );
    ...
    public static string EvaluateCaptionExpression(string caption)
    {
        ...
        IRuleEngineService re =
            ServiceManager.Services.GetService(typeof(IRuleEngineService)) as IRuleEngineService;
    ...
    private void AddTableExtendedProperties(DataTable table, DataStructureEntity entity)
    {
        ...
        table.ExtendedProperties.Add(
            Origam.Constants.SerializeAsSingleJsonObject,
            entity.SerializeAsSingleJsonObject
        );
    }
    ...
    private void AddColumnExtendedProperties(
        DataColumn tableColumn,
        IDataEntityColumn column,
        IDataEntity entity,
        IDataLookup dataLookup
    )
    {
    ...
    private static string AddRelation(
        DataSet dataset,
        string debugPath,
        IAssociation assoc,
    ...
    {
        var entityItems = assoc.ChildItemsByType<EntityRelationColumnPairItem>(
        ...
        for (int i = 0; i < entityItems.Count; i++)
        {
            ...
            string baseFieldName = (entityItems[i]).BaseEntityField.Name;
            string relatedFieldName = (entityItems[i]).RelatedEntityField.Name;
            if (baseTable == null)
            {
                throw new ArgumentOutOfRangeException(
                    "baseEntityName",
                    string.Format(
                        "Cannot create relation {0} > {1}. {0} entity is not present because it contains no fields. DataStructure: {2}.",
                        baseEntityName,
                        relatedEntityName,
                        debugPath
                    )
            ...
            if (relatedTable == null)
            {
                throw new ArgumentOutOfRangeException(
                    "relatedEntityName",
                    string.Format(
                        "Cannot create relation {0} > {1}. {1} entity is not present because it contains no fields. DataStructure: {2}.",
                        baseEntityName,
                        relatedEntityName,
                        debugPath
                    )
            ...
            if (!baseTable.Columns.Contains(baseFieldName))
            {
                throw new ArgumentOutOfRangeException(
                    "baseFieldName",
                    string.Format(
                        "Cannot create relation {0} > {1}. {0} entity does not contain field {2}. DataStructure: {3}.",
                        baseEntityName,
                        relatedEntityName,
                        baseFieldName,
                        debugPath
            ...
            if (!relatedTable.Columns.Contains(relatedFieldName))
            {
                throw new ArgumentOutOfRangeException(
                    "relatedFieldName",
                    string.Format(
                        "Cannot create relation {0} > {1}. {1} entity does not contain field {2}. DataStructure: {3}.",
                        baseEntityName,
                        relatedEntityName,
                        relatedFieldName,
                        debugPath
        ...
        try
        {
            ...
            relation = dataset.Relations.Add(relationName, baseColumns, relatedColumns, false);
        }
        catch (Exception ex)
        {
            throw new Exception(
    ...
    /// <returns></returns>
    private DataStructureEntity LookupRelation(
        DataStructureEntity baseEntity,
        IAssociation relation
    )
    {
        foreach (
            var childEntity in baseEntity.ChildItemsByType<DataStructureEntity>(
                DataStructureEntity.CategoryConst
            )
        )
        {
            if (
                childEntity.Entity.PrimaryKey.Equals(relation.PrimaryKey)
                && childEntity.Columns.Count > 0
            )
            {
                return childEntity;
            }
    ...
    private string RenderAggregation(
    ...
    )
    {
        ...
        if (relatedEntity != null)
        {
            ...
            {
                result =
                    GetAggregationString(aggregationType)
                    + "(Child("
                    + relatedEntity.Name
                    + ")."
                    + columnName
    ...
    private string RenderParentField(DataStructureColumn column)
    {
        ...
        DataStructureEntity requestedParentEntity = column.Entity as DataStructureEntity;
        result.Append("Parent.");
        DataStructureEntity parentEntity =
            (column.ParentItem as DataStructureEntity).ParentItem as DataStructureEntity;
        if (parentEntity.PrimaryKey.Equals(requestedParentEntity.PrimaryKey))
        {
        ...
        else
        {
            ...
            for (int i = 0, len = parentEntity.Columns.Count; i < len; i++)
            {
                ...
                if (
                    (parentEntityColumn.Entity != null)
                    && parentEntityColumn.Entity.PrimaryKey.Equals(requestedParentEntity.PrimaryKey)
                    && parentEntityColumn.Field.PrimaryKey.Equals(column.Field.PrimaryKey)
                )
                {
            ...
            if (matchingColumn == null)
            {
                throw new Exception(
                    ResourceUtils.GetString(
                        "ColumnNotFoundInParentEntity",
                        parentEntity.Name,
                        requestedParentEntity.Name,
                        column.Field.Name
                    )
                );
            }
    ...
    private string RenderExpression(DataConstant constant)
    {
        ...
        if (UserDefinedParameters)
        {
            ...
            {
                value = (string)
                    _parameterService.GetParameterValue(constant.Id, OrigamDataType.String);
            }
        }
        else
        {
    ...
    private string RenderColumn(IDataEntityColumn item, DataStructureEntity entity)
    {
        ...
        {
            if (
                !(column.UseLookupValue && !(item is LookupField))
                && column.Field.PrimaryKey.Equals(item.PrimaryKey)
            )
            {
                return column.Name;
            }
    ...
    private string RenderExpression(FunctionCall item, DataStructureEntity entity)
    {
        ...
        switch (item.Function.Name)
        {
                ...
                result =
                    ...
                    + (
                        item.GetChildByName("Then").ChildItems.Count == 0
                            ? "null"
                            : RenderExpression(item.GetChildByName("Then").ChildItems[0], entity)
                    )
                    + ", "
                    + (
                        item.GetChildByName("Else").ChildItems.Count == 0
                            ? "null"
                            : RenderExpression(item.GetChildByName("Else").ChildItems[0], entity)
                    )
                    + ")";
                break;
            }
            ...
            case "IsNull":
            {
                ...
                result =
                    "ISNULL("
                    + RenderExpression(expressionArg, entity)
                    + ", "
            ...
            case "Between":
            {
                ...
                result =
                    betweenExpression
                    + " >= "
                    + RenderExpression(leftArg, entity)
                    + " AND "
                    + betweenExpression
                    + " <= "
