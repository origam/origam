...
public class FilePersistenceIndex : IDisposable
{
    ...
    private ItemTracker ItemTracker
    {
        get { return readWriteLock.RunReader(() => itemTracker); }
    }
    ...
    {
        readWriteLock.RunWriter(() => binFileLoader.Persist(itemTracker));
    }
    ...
    internal PersistedObjectInfo GetParent(IFilePersistent instance) =>
        instance.FileParentId == Guid.Empty ? null : GetById(instance.FileParentId);

    internal IEnumerable<PersistedObjectInfo> GetByParentId(Guid parentId)
    {
        return readWriteLock.RunReader(() =>
            itemTracker.GetByParentId(parentId).Where(BelongsToALoadedPackage)
        );
    }

    internal IEnumerable<PersistedObjectInfo> GetByParentFolder(string category, Guid folderId)
    {
        return readWriteLock.RunReader(() =>
            itemTracker.GetByParentFolder(category, folderId).Where(BelongsToALoadedPackage)
        );
    }

    internal IEnumerable<PersistedObjectInfo> GetListByCategory(string category)
    {
        return readWriteLock.RunReader(() =>
            itemTracker.GetListByCategory(category).Where(BelongsToALoadedPackage)
        );
    }

    internal IEnumerable<PersistedObjectInfo> GetByPackage(Guid packageId)
    {
        return readWriteLock.RunReader(() => itemTracker.GetByPackage(packageId));
    }
    ...
    {
        return LoadedPackages.ToDictionary(id => id, FindPackageDirectory);
    }
    ...
    public DirectoryInfo FindPackageDirectory(Guid packageId)
    {
        return readWriteLock.RunReader(() =>
            itemTracker
                .PackegeFiles.First(orFile => orFile.ContainedObjects.Keys.Contains(packageId))
                .Path.Directory
        );
    }
    ...
    {
        return readWriteLock.RunReader(() => itemTracker.GetByPath(instanceRelativeFilePath));
    }

    public void InitItemTracker(
    ...
    {
        readWriteLock.RunWriter(() => itemTracker.RenameDirectory(dirToRename, newDirPath));
    }

    public void AddOrReplaceHash(ITrackeableFile origamFile)
    {
        readWriteLock.RunWriter(() => itemTracker.AddOrReplaceHash(origamFile));
    }
    ...
    {
        return readWriteLock.RunReader(() => itemTracker.HasFile(newRelativePath));
    }
    ...
    {
        return readWriteLock.RunReader(() => itemTracker.GetByDirectory(dir));
    }
}
