...
class Version6PropertyToNamespaceMapping : PropertyToNamespaceMapping
{
    private static readonly ConcurrentDictionary<
        Type,
        Version6PropertyToNamespaceMapping
    > instances = new ConcurrentDictionary<Type, Version6PropertyToNamespaceMapping>();
    private static readonly Version version6 = new Version(6, 0, 0);

    public static Version6PropertyToNamespaceMapping CreateOrGet(
        Type instanceType,
        ScriptContainerLocator scriptLocator
    )
    {
        return instances.GetOrAdd(
            ...
            type =>
            {
                ...
                var propertyMappings = GetPropertyMappings(instanceType, scriptLocator);
                return new Version6PropertyToNamespaceMapping(propertyMappings, typeFullName);
            }
        );
    }

    protected Version6PropertyToNamespaceMapping(
    ...
    {
        var oldPropertyNameDict =
            scriptLocator.TryFindByTypeName(type.FullName)?.OldPropertyXmlNames ?? new string[0];
        var oldPropertyNames = oldPropertyNameDict.Select(oldName => new PropertyName
        {
            Name = "",
