...
public class DeadModelFilesChecker : IFileSystemModelChecker
{
    ...
    public IEnumerable<ModelErrorSection> GetErrors()
    {
        ...
        IFilePersistent[] allPersistedObjects = filePersistenceProvider
            ...
            .ToArray();
        var modelFilePaths = allPersistedObjects.Select(persistent =>
            Path.Combine(topDirectoryPath, persistent.RelativeFilePath)
        );
        ...
        HashSet<string> knownFilePaths = new HashSet<string>(
            modelFilePaths.Concat(externalFilePaths)
        );
