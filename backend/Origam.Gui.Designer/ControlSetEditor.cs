...
public class ControlSetEditor : AbstractEditor
{
	...
	public ControlSetEditor()
	{
		...
		_dsProvider = _schema.GetProvider(typeof(DataStructureSchemaItemProvider)) as DataStructureSchemaItemProvider;
		_deProvider = _schema.GetProvider(typeof(EntityModelSchemaItemProvider)) as EntityModelSchemaItemProvider;
	...
	private void UpdateDataSource(object control)
	{
        ...
        if (panel != null && string.IsNullOrEmpty(panel.DataMember)
            && _dataSourceMode == eDataSource.DataStructure)
        {
            foreach (DataTable table in _origamData.Tables)
            {
                Guid entityId = ((ControlSetItem)panel.Tag).ControlItem.PanelControlSet.DataSourceId;
	...
	private void AddDataset()
	{
		...
		{
			System.Windows.Forms.Design.ComponentTray tray = provider as System.Windows.Forms.Design.ComponentTray;
			if(tray != null)
			{
				tray.Hide();
			}
	...
	private void ControlProperties(Control cntrl, bool save)
	{
		if(cntrl.Tag is ControlSetItem)
		{
			...
			foreach(var propItem in cntrSetItem.ControlItem.ChildItemsByType<ControlPropertyItem>(ControlPropertyItem.CategoryConst))
			{
				...
				if(property == null)
                {
                    throw new ArgumentOutOfRangeException("Name", propItem.Name, "Property '" + propItem.Name + "' not found in the control '" + t.ToString() + "'");
                }
	...
	private void LoadControlBindings(Control cntrl)
	{
		...
		foreach(var bindItem in cntrSetItem.ChildItemsByType<PropertyBindingInfo>(PropertyBindingInfo.CategoryConst))
		{
			try
			{
				...
				{
					cntrl.DataBindings.Add(bindItem.ControlPropertyItem.Name,_origamData, _origamData.Tables[0].TableName + "." + bindItem.Value);
				}
				else
				{
					cntrl.DataBindings.Add(bindItem.ControlPropertyItem.Name,_origamData,bindItem.DesignDataSetPath);
				}
			}
			catch
			{
			...
			if(CheckControlItemForToolbox(item))
			{
				...
				if(kind == KindOfTool.Basic && (!item.IsComplexType) )
				{
					...
					if(this.IsPanel &&
                        ...
                        || item.ControlToolBoxVisibility == ControlToolBoxVisibility.PanelAndFormDesigner))
					{
				...
				{
					if(item.IsComplexType && item.ControlToolBoxVisibility != ControlToolBoxVisibility.Nowhere)
					{
						FDToolboxItem fd_item = new FDToolboxItem();
						fd_item.Type =item.ControlType + ","  + item.ControlNamespace;
		...
		if (_propertyGrid != null && selectionService != null)
		{
			...
			if(selectedObject != null)
			{
				...
				if(ctrlSet != null)
				{
					...
					{
						settings["DataSource"] = new Setting((selectedObject as AsPanel).DataSource, typeof(object));
					}
					// Add bindings, we want to see this always
					settings["DataBindings"] = new Setting(selectedObject.DataBindings, selectedObject.DataBindings.GetType());
					// Add any typeconverters
					IList members = Reflector.FindMembers(t, typeof(System.ComponentModel.TypeConverterAttribute), new Type[0]);
					foreach(MemberAttributeInfo mi in members)
					{
						System.ComponentModel.TypeConverterAttribute converter = mi.Attribute as System.ComponentModel.TypeConverterAttribute;
					...
					members = Reflector.FindMembers(t, typeof(System.ComponentModel.EditorAttribute), new Type[0]);
					foreach(MemberAttributeInfo mi in members)
					{
						System.ComponentModel.EditorAttribute editor = mi.Attribute as System.ComponentModel.EditorAttribute;
						if(settings[mi.MemberInfo.Name] != null)
						{
	...
	private ControlItem FindControlItemByPanelRef(PanelControlSet panel)
	{
        ...
        foreach (ControlItem item in _controls.ChildItems)
		{
			if(item.PanelControlSet !=null && item.PanelControlSet.PrimaryKey.Equals(panel.PrimaryKey))
            {
                return item;
            }
        }
	...
	private void CreateNewControlSetItem(Control control, string name, ControlSetItem parent)
	{
		...
		if(refControl!=null)
		{
			...
			if(_selectedComponents !=null && _selectedComponents.Length>0 && _selectedComponents[0] is Control)
			{
	...
	private void DebugBindings(object formik)
	{
		foreach(Control ctrl in ((Control)formik).Controls)
		{
			foreach(Binding bind in ctrl.DataBindings)
			{
				...
				System.Diagnostics.Debug.WriteLine("ControlType:       " + bind.Control.GetType().ToString());
				System.Diagnostics.Debug.WriteLine("Property:          " + bind.PropertyName.ToString());
				System.Diagnostics.Debug.WriteLine("BindingMemberInfo: " + bind.BindingMemberInfo.ToString());
				System.Diagnostics.Debug.WriteLine("Field:             " + bind.BindingMemberInfo.BindingField.ToString());
				System.Diagnostics.Debug.WriteLine("BindingPath:       " + bind.BindingMemberInfo.BindingPath.ToString());
				System.Diagnostics.Debug.WriteLine("Member:            " + bind.BindingMemberInfo.BindingMember.ToString());
				System.Diagnostics.Debug.WriteLine("***************************************************************************");
			}
			if(ctrl.Controls.Count > 0){DebugBindings(ctrl);}
		}
	}
