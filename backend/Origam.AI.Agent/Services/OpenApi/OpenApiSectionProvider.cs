...
public class OpenApiSectionProvider(ArchitectApiClient architectApi, TargetOptions options)
{
    ...
    public string BaseUrl => architectApi.BaseUrl;

    public string? LastError { get; private set; }

    public IReadOnlyList<string> TagsFor(string sectionName)
    {
        var tags = new List<string>();
        if (!options.SectionsOutOfBeta.Contains(sectionName))
        {
            tags.Add(BetaTag);
        }

        tags.AddRange(
            options.AdditionalSectionTags.GetValueOrDefault(sectionName, Array.Empty<string>())
        );
        return tags;
    }
    ...
    )
    {
        ...
        return sections!
            ...
            .Select(section => new SectionInfo(
                section.Name,
                options.SectionDescriptions.GetValueOrDefault(section.Name, string.Empty),
                ...
                section
                    ...
                    .ToArray(),
                options.DefaultSections.Contains(section.Name)
            ))
            .ToArray();
    }
    ...
    private async Task<bool> EnsureLoadedAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            swaggerBytes = await architectApi.GetOpenApiDocumentAsync(cancellationToken);
    ...
    private Dictionary<string, ApiSection> ParseSections(byte[] bytes)
    {
        ...
        if (document.RootElement.TryGetProperty(propertyName: "paths", out var paths))
        {
            foreach (var pathEntry in paths.EnumerateObject())
            {
                ...
                if (options.PathsNeverExposedAsTools.Contains(path))
                {
                    continue;
                }
                ...
                foreach (var methodEntry in pathEntry.Value.EnumerateObject())
                {
                    ...
                    if (options.SectionsNeverExposedAsTools.Contains(tag))
                    {
                        continue;
                    }
    ...
    private static string ReadDescription(JsonElement operation)
    {
        ...
        {
            var value = operation.GetStringOrNull(propertyName);
