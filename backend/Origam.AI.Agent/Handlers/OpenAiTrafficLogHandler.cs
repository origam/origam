...
public class OpenAiTrafficLogHandler(ILogger<OpenAiTrafficLogHandler> logger) : DelegatingHandler
{
    ...
    private static string DescribeRequest(string body)
    {
        ...
        try
        {
            ...
            {
                index++;
                var role = message.GetStringOrNull(propertyName: "role") ?? "?";
        ...
        try
        {
            ...
            if (root.TryGetProperty(propertyName: "usage", out var usageElement))
            {
                var cached = usageElement.TryGetProperty(
                    ...
                    ? promptDetails.GetInt32OrNull(propertyName: "cached_tokens") ?? 0
                    : 0;
                usage =
                    $", {usageElement.GetInt32OrNull(propertyName: "prompt_tokens") ?? 0} prompt"
                    + $" ({cached} cached)"
                    + $" + {usageElement.GetInt32OrNull(propertyName: "completion_tokens") ?? 0} completion tokens";
            }
    ...
    private static void AppendToolCalls(StringBuilder builder, JsonElement message)
    {
        ...
        foreach (var toolCall in toolCalls.EnumerateArray())
        {
            ...
            var name = function.GetStringOrNull(propertyName: "name") ?? "?";
            var arguments = function.GetStringOrNull(propertyName: "arguments") ?? string.Empty;
