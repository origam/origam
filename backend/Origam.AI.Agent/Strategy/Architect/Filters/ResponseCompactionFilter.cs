...
public class ResponseCompactionFilter(NewItemTypeCatalogService catalogService)
    : IToolInvocationFilter
{
    ...
    {
        string? itemTypeName = node.GetStringOrNull(propertyName: "itemTypeName");
        if (
        ...
        {
            ["id"] = node.GetStringOrNull(propertyName: "origamId"),
    ...
    )
    {
        ...
        foreach (JsonElement property in properties.EnumerateArray())
        {
            ...
            string? name =
                property.GetStringOrNull(propertyName: "name")
                ?? property.GetStringOrNull(propertyName: "propertyName");
            if (name is null)
            {
                return false;
            }
            ...
            if (truncated || property.GetBooleanOrNull(propertyName: "readOnly") == true)
            {
                continue;
            }
    ...
    {
        string kind = property.GetStringOrNull(propertyName: "type") switch
        {
            "looukup" => "reference",
            "enum" => "enum",
            _ => string.Empty,
