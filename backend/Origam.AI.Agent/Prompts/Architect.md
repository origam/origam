...
The business entities in the current ORIGAM project, grouped by package, in a compact notation. Format: a line 'Name(id,K)' starts an entity, where K is D for a database entity and V for a virtual entity. The lines that follow list, comma separated inside brackets, the entity's fields on the 'f' line and then the model items that use the entity, one line per kind of item labelled with the kind's name as it appears in the Architect (for example 'Data Structure', 'Screen', 'Screen Section', 'Data Service Lookup', 'Work Queue Class', 'Sequential Workflow'). Related items are written as 'Name=id'. A '*' after a field name marks it as primary key, and that field is written as 'Name*=id' - use that id as ForeignKeyField when another entity points at this one. A line is omitted when the entity has nothing of that kind. Standard audit fields (RecordCreated, RecordCreatedBy, RecordCreatedServer, RecordUpdated, RecordUpdatedBy, RecordUpdatedServer, _mockPk, Selected) are left out of the f lines to keep this index short; call ExploreNode on the entity if you need to confirm them. Use these ids directly in tool calls; you do NOT need to call SearchSchema for anything listed here.
