...
public class DiffInfoMargin : AbstractMargin
{
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        _lineFt = new FormattedText(
            ...
            tf,
            (double)GetValue(TextBlock.FontSizeProperty),
            BackBrush,
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        foreach (var v in visualLines)
        {
            var rcs = BackgroundGeometryBuilder
                .GetRectsFromVisualSegment(TextView, v, 0, 1000)
                .ToArray();
            ...
            if (diffLine.Style != DiffContext.Context)
            {
                ...
                foreach (var rc in rcs)
                {
                    drawingContext.DrawRectangle(
                        brush,
                        BorderlessPen,
                        new Rect(0, rc.Top, ActualWidth, rc.Height)
                    );
                }
            }
            if (diffLine.Text != "")
            ...
            if (diffLine.PrefixForStyle != "")
            {
                ...
                ft = new FormattedText(
                    ...
                    tf,
                    fontSize,
                    (Brush)TextView.GetValue(Control.ForegroundProperty),
                    VisualTreeHelper.GetDpi(this).PixelsPerDip
                );
                drawingContext.DrawText(
                    ft,
                    new Point(lineNumberWidth + TextHorizontalMargin, rcs[0].Top)
                );
            }
        }
    }
