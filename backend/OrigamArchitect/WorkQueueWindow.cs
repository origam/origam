...
public class WorkQueueWindow : AsForm
{
	...
	private void BuildUI(WorkQueueClass wqc, Guid queueId)
	{
			...
			int lastPos = 0;
			foreach(DataStructureColumn col in ((DataStructureEntity)wqc.WorkQueueStructure.Entities[0]).Columns)
			{
				if(col.Name != "Id" & col.Name != "RecordCreated" & col.Name != "RecordUpdated" & col.Name != "RecordCreatedBy" & col.Name != "RecordUpdatedBy" & col.Name != "refWorkQueueId")
				{
					lastPos += 20;
	...
	private void link_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
	{
			...
			if(grid != null && grid.DataSource != null)
			{
				...
				{
					IWorkQueueService wqs = ServiceManager.Services.GetService(typeof(IWorkQueueService)) as IWorkQueueService;
					wqs.HandleAction((Guid)row["Id"],
                        this.QueueClass, resultTable, (Guid)row["refWorkQueueCommandTypeId"], 
				...
				{
					AsMessageBox.ShowError(this, ex.Message, strings.ErrorWhenProcessingWorkQueueCommand_Message, ex);
				}
