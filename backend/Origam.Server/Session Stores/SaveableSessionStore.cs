...
public abstract class SaveableSessionStore : SessionStore
{
    ...
    public Guid DataStructureId
    {
        ...
        set
        {
            ...
            if (_dataStructureId != Guid.Empty)
            {
                ...
                {
                    DirtyEnabledEntities.Add(entity.Name);
                }
            }
        }
    }
    ...
    {
        if (Template == null || !Template.Entity.Name.Equals(entity))
        {
        ...
        {
            key = TemplateTools.AddTemplateRecord(null, Template, entity, DataStructureId, Data);
        }
        else
        {
    ...
    )
    {
        ...
        if (_entityFieldDependencies.ContainsKey(dsEntityId))
        {
            if (_entityFieldDependencies[dsEntityId].ContainsKey(fieldId))
            {
                foreach (Guid dependentColumnId in _entityFieldDependencies[dsEntityId][fieldId])
                {
                    ...
                    try
                    {
                        UpdateObjectWithDependenies(entity, id, dependentColumnName, null, false);
                    }
        ...
        UpdateObjectsWithoutGetChanges(entity, id, property, newValue);
