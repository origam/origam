...
public class OpenIddictTokenStore : IOpenIddictTokenStore<OpenIddictToken>
{
    ...
    public IQueryable<OpenIddictToken> Tokens
    {
        get
        {
            Task<IEnumerable<OpenIddictToken>> allTokens = GetAllAsync(new OpenIddictTokenFilter());
            return allTokens.Result.AsQueryable();
        }
    }

    public async ValueTask<long> CountAsync(CancellationToken cancellationToken)
    {
        IEnumerable<OpenIddictToken> tokens = await GetAllAsync(new OpenIddictTokenFilter()).ConfigureAwait(false);
        return tokens.LongCount();
    }

    public async ValueTask<long> CountAsync<TResult>(Func<IQueryable<OpenIddictToken>, IQueryable<TResult>> query, CancellationToken cancellationToken)
    {
        IEnumerable<OpenIddictToken> tokens = await GetAllAsync(new OpenIddictTokenFilter()).ConfigureAwait(false);
        return query(tokens.AsQueryable()).LongCount();
    }

    public async ValueTask<OpenIddictToken> FindByReferenceIdAsync(string identifier, CancellationToken cancellationToken)
    {
        var filter = new OpenIddictTokenFilter { SessionId = identifier };
        IEnumerable<OpenIddictToken> tokens = await GetAllAsync(filter).ConfigureAwait(false);
        return tokens.FirstOrDefault();
    }

    public async IAsyncEnumerable<OpenIddictToken> FindByApplicationIdAsync(string identifier, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        var filter = new OpenIddictTokenFilter { ClientId = identifier };
        IEnumerable<OpenIddictToken> tokens = await GetAllAsync(filter).ConfigureAwait(false);
        foreach (OpenIddictToken token in tokens)
        {
            yield return token;
        }
    }

    public IAsyncEnumerable<OpenIddictToken> FindByAuthorizationIdAsync(string identifier, CancellationToken cancellationToken)
    {
        return AsyncEnumerable.Empty<OpenIddictToken>();
    }

    public ValueTask<string> GetApplicationIdAsync(OpenIddictToken token, CancellationToken cancellationToken) => new(token.ClientId);

    public ValueTask<string> GetAuthorizationIdAsync(OpenIddictToken token, CancellationToken cancellationToken) => new(string.Empty);
    ...
    public ValueTask<string> GetStatusAsync(OpenIddictToken token, CancellationToken cancellationToken) => new(string.Empty);
    ...
    public ValueTask SetAuthorizationIdAsync(OpenIddictToken token, string identifier, CancellationToken cancellationToken) => ValueTask.CompletedTask;
    ...
    public ValueTask SetStatusAsync(OpenIddictToken token, string status, CancellationToken cancellationToken) => ValueTask.CompletedTask;
    ...
    public ValueTask UpdateAsync(OpenIddictToken token, CancellationToken cancellationToken)
    {
        DataSet dataSet = DataService.Instance.LoadData(
            OrigamIdentityGrantDataStructureId,
            GetGrantByKeyFilterId,
        ...
        if (dataSet.Tables[GrantTableName].Rows.Count == 0)
        {
            throw new ArgumentException($"Grant {token.Id} not found.");
        }

        DataRow row = dataSet.Tables[GrantTableName].Rows[0];
        row.SetField("Type", token.Type);
        row.SetField("SubjectId", token.Subject);
        row.SetField("ClientId", token.ClientId);
        row.SetField("CreationTime", token.CreationDate.UtcDateTime);
        row.SetField("Expiration", token.ExpirationDate?.UtcDateTime);
        row.SetField("Data", token.Payload);
        row.SetField("SessionId", token.ReferenceId);
    ...
    {
        IEnumerable<OpenIddictToken> tokens = await GetAllAsync(new OpenIddictTokenFilter()).ConfigureAwait(false);
        foreach (OpenIddictToken token in tokens.Where(t => t.ExpirationDate.HasValue && t.ExpirationDate <= threshold))
        {
            await RemoveAsync(token.Id, cancellationToken).ConfigureAwait(false);
        }
    }
