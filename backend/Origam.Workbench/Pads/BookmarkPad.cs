...
{
	public class BookmarkPad : AbstractResultPad
	{
		...
		public BookmarkPad()
		{
			...
			_schemaBrowser = WorkbenchSingleton.Workbench.
				GetPad(typeof(SchemaBrowser)) as SchemaBrowser;
			lvwResults.SmallImageList =
				_schemaBrowser.EbrSchemaBrowser.imgList;
		...
		private void OnColumnClick(object sender, ColumnClickEventArgs eventArgs)
		{
			...
			this.lvwResults.ListViewItemSorter = new ListViewItemComparer(
				eventArgs.Column, lvwResults.Sorting);
			lvwResults.Sort();
		}
		...
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(BookmarkPad));
			...
			this.lvwResults.Size = new System.Drawing.Size(1018, 245);
			...
			this.ClientSize = new System.Drawing.Size(1018, 245);
		...
		public void AddResult(AbstractSchemaItem result)
		{            
            List<Guid> referencePackages = new List<Guid>();
            TreeNode treenode = (WorkbenchSingleton.Workbench.
				GetPad(typeof(SchemaBrowser))as SchemaBrowser).
				EbrSchemaBrowser.GetFirstNode();
            if (treenode != null)
            {
                var itm = (Package)treenode.Tag;
                referencePackages = itm.IncludedPackages.
					Select(x =>{ return x.Id; }).ToList();
                referencePackages.Add(itm.Id);
            }
            
            ListViewItem resultListItem = new ListViewItem();
            
            var item = result;
            resultListItem = GetResult(item, referencePackages);
            _results.Add(item);
            
            lvwResults.Items.Add(resultListItem);
		...
		private ListViewItem GetResult(AbstractSchemaItem item,
			List<Guid> referencePackages)
		{
			if(item == null) return null;

			if(! LicensePolicy.ModelElementPolicy(item.GetType().Name,
				ModelElementPolicyCommand.Show))
			{
				return null;
			}
			...
			ListViewItem newItem = new ListViewItem(new string[]
				{item.Path, rootName, name,	item.RootItem.Group == null
				? "" : item.RootItem.Group.Path,item.PackageName,
                referencePackages.Contains(item.SchemaExtensionId) ?
				"Yes":"No"});
			newItem.Tag = item;
		...
		private void ActivateItem()
		{
			if(lvwResults.SelectedItems.Count > 0)
			{
				try
				{
                    AbstractSchemaItem schemaItem = lvwResults.
						SelectedItems[0].Tag as AbstractSchemaItem;
		...
		private void lvwResults_KeyDown(object sender,
