...
public class SchemaCompareEditor : AbstractViewContent
{
	...
	public void DisplayResults(ArrayList results)
	{
		...
		{
			ListViewItem item = new ListViewItem(new string[] {SchemaItemDescription(result.SchemaItemType),
															result.ItemName,
	...
	{
		if((lvwResults.Items[e.Index].Tag as SchemaDbCompareResult).ResultType == DbCompareResultType.ExistingButDifferent)
		{
	...
	{
		Pads.OutputPad _pad = WorkbenchSingleton.Workbench.GetPad(typeof(Pads.OutputPad)) as Pads.OutputPad;
		...
		{	
			...
			{
				if(result.ResultType == DbCompareResultType.MissingInDatabase & result.Script != "")
				{
			...
			foreach(SchemaDbCompareResult result in SelectedResults())
			{
				if(result.ResultType == DbCompareResultType.MissingInDatabase & result.Script2 != "")
				{
