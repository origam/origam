...
public class PackageEditor : AbstractViewContent
{
	...
	private void InitializeComponent()
	{
		...
		this.colReferencedPackage = ((System.Windows.Forms.ColumnHeader)(new System.Windows.Forms.ColumnHeader()));
	...
	private static void RefreshPads()
	{
		SchemaBrowser modelBrowser =
			WorkbenchSingleton.Workbench.GetPad(typeof(SchemaBrowser)) as
				SchemaBrowser;
		modelBrowser?.EbrSchemaBrowser.RefreshRootNodeText();
		ExtensionPad packageBrowser =
			WorkbenchSingleton.Workbench.GetPad(typeof(ExtensionPad)) as
				ExtensionPad;
		...
		get
		{
			if(lvwReferences.SelectedItems.Count == 1)
			{
				...
				if(reference.IsPersisted)
				{
					try
					{
						return (lvwReferences.SelectedItems[0].Tag as PackageReference).ReferencedPackage;
					}
					catch
					{
						return null;
	...
	void toolBar1_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
	{
		try
		{
			...
			if(e.ClickedItem == btnDelete)
			{
				...
				{
					PackageReference reference = lvwReferences.SelectedItems[0].Tag as PackageReference;
					if(reference.IsPersisted == false || IsPackageReferenced(SelectedReferencedPackage) == false)
					{
					...
					{
						throw new InvalidOperationException(ResourceUtils.GetString("ErrorRemovePackageReference"));
					}
				}
			}
