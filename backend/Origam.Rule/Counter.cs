...
public class Counter : ICounter
{
		...
		string transactionId)
	{
		...
		{
			throw new ArgumentOutOfRangeException("counterCode", counterCode, ResourceUtils.GetString("ErrorNoSequence"));
		}
		else if(_data.Counter[0].ManageValidityByDate)
		{
			foreach(CounterDataset.CounterDetailRow row in _data.CounterDetail)
			{
				if(row.IsValidFromNull() == false && row.IsValidToNull() == false && (row.ValidFrom <= date & row.ValidTo >= date))
				{
					return row;
				}
			}
			throw new Exception(ResourceUtils.GetString("ErrorNoSequenceForDate", date, counterCode));
		}
		else
		{
			...
			if(rows.Length == 0)
			{
				throw new ArgumentOutOfRangeException("counterCode", counterCode, "Èíselná øada existuje, ale nemá definován rozsah.");
			}
			return rows[0];
		}
	}
