...
public class FilePersistenceProviderTestWithOrigamRuntime : AbstractFileTestClass
{
    ...
    {
        OrigamEngine.OrigamEngine.ConnectRuntime("Data for Xml serialization Test");
    ...
    {
        foreach (string dir in Directory.EnumerateDirectories(settings.ModelSourceControlLocation))
        {
            Directory.Delete(dir, true);
        }
        foreach (string file in Directory.EnumerateFiles(settings.ModelSourceControlLocation))
        {
            File.Delete(file);
        }
    }
