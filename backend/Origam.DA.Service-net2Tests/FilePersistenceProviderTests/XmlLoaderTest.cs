...
public class XmlLoaderTest : AbstractFileTestClass
{
    ...
    private static readonly XmlFileDataFactory XmlFileDataFactory = new XmlFileDataFactory();
    ...
    public void ReadPackageFile()
    {
        ...
        var packageFileData = objectFileDataFactory.NewPackageFileData(
            XmlFileDataFactory.Create(new FileInfo(fileName)).Value
        );
        Guid packageId = packageFileData.PackageId;
        var expectedPackageId = new Guid("3e37fa44-cdb7-4804-8176-8df118a918ae");
    ...
    public void ReadGroupFile()
    {
        ...
        var expectedPackageId = new Guid("d266feb3-ff9e-4ac2-8386-517a31519d06");
    ...
    public void ReadReferenceFile()
    {
        ...
        var referenceFileData = objectFileDataFactory.NewReferenceFileData(xmlFileData);
    ...
    public void ReadObjectFile()
    {
        ...
        var objectFileData = new ObjectFileData(
            ...
            XmlFileDataFactory
                .Create(new FileInfo(Path.Combine(TestFilesDir.FullName, "MultiEntityTest.origam")))
                .Value,
            MakeOrigamFileFactory(TestFilesDir)
        );

        objectFileData.ParentFolderIds[OrigamFile.PackageCategory] = new Guid(
            "e002a017-75b8-4f6e-8539-576ca05d6952"
        );
    ...
    private void InitFilePersistenceProvider(List<string> parentFolders, DirectoryInfo topDir)
    {
        var ignoredFileFilter = new FileFilter(
            new HashSet<string> { "bin", "bak" },
            new FileInfo[0],
            new string[0]
        );
        var fileChangesWatchDog = new FileChangesWatchDog(topDir: topDir, ignoredFileFilter);
        var pathToIndexBin = new FileInfo(Path.Combine(topDir.FullName, "index.bin"));
        var pathFactory = new OrigamPathFactory(TestProjectDir);
        var index = new FilePersistenceIndex(pathFactory);
        var fileEventQueue = new FileEventQueue(index, fileChangesWatchDog);
        var nullEventQueue = new FileEventQueue(index, new NullWatchDog());
        ...
        var objectFileDataFactory = new ObjectFileDataFactory(origamFileFactory, parentFolders);
        ...
        var filePersistenceProvider = new FilePersistenceProvider(
            topDir,
            fileEventQueue,
            ignoredFileFilter,
