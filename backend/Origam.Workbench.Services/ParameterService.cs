...
public class ParameterService : IParameterService
{
	...
	public string GetString(string name, bool throwException, params object[] args)
	{
			...
			{
                if (throwException)	throw new ArgumentOutOfRangeException("name", name, ResourceUtils.GetString("ErrorStringNotFound"));
                return "";
			}
		}
                ...
                if (_userDefinableConstants.ContainsKey(id))
                {
                    ...
                    OrigamParametersData.OrigamParametersRow row =
                        _parameterData.OrigamParameters.FindByProfileIdId(
							((overridenProfileId != null) ? overridenProfileId.Value
	...
	public object GetParameterValue(string parameterName, Guid? overridenProfileId = null)
	{
			...
			{
				throw new ArgumentOutOfRangeException("parameterName", parameterName, ResourceUtils.GetString("ConstantNotFound"));
			}
		}
	...
	public object GetCustomParameterValue(Guid id)
	{
			...
			switch(constant.DataType)
			{
				...
				case OrigamDataType.String:
					...
					break;
				default:
					throw new ArgumentOutOfRangeException("dataType", constant.DataType, "Type not supported for custom parameters. Cannot retrieve value.");
			}
		...
		decimal currencyValue, object dateValue, bool useIdentity, Guid? overridenProfileId)
	{
			if(_constantIdByName.ContainsKey(parameterName))
			{
				SetCustomParameterValue(_constantIdByName[parameterName], 
					value, guidValue, intValue, stringValue, boolValue, floatValue, 
					currencyValue, dateValue, useIdentity, overridenProfileId);
			}
			else
			{
				throw new ArgumentOutOfRangeException("parameterName", parameterName, ResourceUtils.GetString("ConstantNotFound"));
            ...
            else
            {
                if (! useIdentity && isUserDefinable)
                {
                    throw new Exception(string.Format(
                        "Constant {0} is set to be user definable. Cannot set its value at this time.", id));
                }
	...
	public void RefreshParameters()
	{
			...
			LoadFeatures();
			LoadStrings();
			DataConstantSchemaItemProvider constants = _schemaService.GetProvider(
                typeof(DataConstantSchemaItemProvider)) as DataConstantSchemaItemProvider;
			if(constants == null) throw new InvalidOperationException(
		...
		OrigamParametersData.OrigamParametersRow row)
	{
            ...
            if (!row.IsStringValueNull()) stringValue = row.StringValue;
            if (!row.IsIntValueNull()) intValue = row.IntValue;
            if (!row.IsCurrencyValueNull()) currencyValue = row.CurrencyValue;
            if (!row.IsFloatValueNull()) floatValue = row.FloatValue;
            if (!row.IsGuidValueNull()) guidValue = row.GuidValue;
            if (!row.IsDateValueNull()) dateValue = row.DateValue;
            return DataConstant.ConvertValue(
                dataType, stringValue, intValue, guidValue,
	...
	public bool IsFeatureOn(string features)
	{
			...
			foreach(string feature in features.Split(";".ToCharArray()))
			{
				...
				{
					throw new ArgumentOutOfRangeException("featureCode", featureName, ResourceUtils.GetString("ErrorFeatureNotFound"));
				}
	...
	public void PrepareParameters()
	{
			lock(_constantsById)
			{
				...
				DataConstantSchemaItemProvider constants = _schemaService.GetProvider(typeof(DataConstantSchemaItemProvider)) as DataConstantSchemaItemProvider;
	...
	private void LoadParameters()
	{
			lock(_parameterLock)
			{
                ...
                if (_parameterData == null)
                {
                    throw new Exception("Old version of Root model detected. Please upgrade to Root version 5.0 or higher.");
                }
			}
		}
	...
	private void LoadFeatures()
	{
			lock(FeatureList)
			{
				...
				FeatureSchemaItemProvider provider = _schemaService.GetProvider(typeof(FeatureSchemaItemProvider)) as FeatureSchemaItemProvider;
				...
				IServiceAgent dataServiceAgent = (ServiceManager.Services.GetService(typeof(IBusinessServicesService)) as IBusinessServicesService).GetAgent("DataService", null, null);
				DataStructureQuery query = new DataStructureQuery(new Guid("278cce8c-f0d2-4738-89f4-d85b1c9ce870"));
				query.LoadByIdentity = false;
