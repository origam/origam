...
public class SchemaActionsMenuBuilder : ISubmenuBuilder
{
	...
	public AsMenuCommand[] BuildSubmenu(object owner)
	{
        ...
        CreateMenuItem(list, "Generate Entity Fields", new GenerateDataStructureEntityColumns(), null);
        ...
        CreateMenuItem(list, "Create Relationship with key...", new CreateFieldWithRelationshipEntityCommand(), null);
		...
		CreateMenuItem(list, "Create Menu Item...", new CreateMenuFromSequentialWorkflowCommand(), null);
		CreateMenuItem(list, "Create Data Structure", new CreateDataStructureFromEntityCommand(), null);
		CreateMenuItem(list, "Create Child Entity...", new CreateChildEntityCommand(), null);
		CreateMenuItem(list, "Create Localization Child Entity...", new CreateLanguageTranslationEntityCommand(), null);
		...
		CreateMenuItem(list, "Create (=) Filter With Parameter", new CreateFilterWithParameterByFieldCommand(), null);
		CreateMenuItem(list, "Create (Like) Filter", new CreateFilterLikeByFieldCommand(), null);
		CreateMenuItem(list, "Create (Like) Filter With Parameter", new CreateFilterLikeWithParameterByFieldCommand(), null);
		CreateMenuItem(list, "Create (List) Filter With Parameter", new CreateFilterByListWithParameterByFieldCommand(), null);
		CreateMenuItem(list, "Create (Between) Filter With Parameters", new CreateFilterBetweenWithParameterByFieldCommand(), null);
		CreateMenuItem(list, "Create Foreign Key...", new CreateForeignKeyCommand(), null);
		CreateMenuItem(list, "Create Work Queue Class...", new CreateWorkQueueClassFromEntityCommand(), null);
...
//			CreateMenuItem(list, "Generate XAML...", new Commands.GenerateXamlCommand(), null);
		CreateMenuItem(list, "Generate Mappings...", new GenerateWorkQueueClassEntityMappings(), null);
//			CreateMenuItem(list, "Show Editor XML", new ShowEditorXml(), null);
	...
	private void MenuItemClick(object sender, EventArgs e)
	{
	    ...
	    try
		{
            ...
            {
                FindSchemaItemResultsPad findResults = WorkbenchSingleton.Workbench.GetPad(
                    typeof(FindSchemaItemResultsPad)) as FindSchemaItemResultsPad;
                findResults.DisplayResults(cmd.Command.GeneratedModelElements.ToArray());
                MessageBox.Show(strings.ModelElementsUpdate_Message,
                                    strings.Results_Title, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
			// offer to execute any newly created deployment scripts
            List<AbstractUpdateScriptActivity> activities = new List<AbstractUpdateScriptActivity>();
            ...
            if (activities.Count > 0 && 
                MessageBox.Show(strings.ExecuteDeploymentScripts_Question,
                strings.DeploymentScripts_Title, MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                == DialogResult.Yes)
            {
                IDeploymentService deploymentService = ServiceManager.Services.GetService(
