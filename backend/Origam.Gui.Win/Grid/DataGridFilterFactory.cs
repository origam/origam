	...
	public class DataGridFilterFactory : IDisposable
	{
		...
		public void AddControlToPanel(Control control, DataColumn column, int controlWidth)
		{
            ...
            if (dropDown != null)
			{
				if(dropDown.ParameterMappings.Count > 0)
				{
					string lookUpColumnName = DatasetTools.SortColumnName(
                        column.ColumnName);
					part = new StringFilterPart(tb, typeof(string), 
                        lookUpColumnName, column.ColumnName, caption, 
                        formGenerator);
				}
				else
				{
					part = new DropDownFilterPart(dropDown, column.DataType, column.ColumnName, column.ColumnName, caption, formGenerator);
				}
			}
			else if(textBox != null)
			{
				...
				{
					part = new StringFilterPart(textBox, column.DataType, column.ColumnName, column.ColumnName, caption, formGenerator);
				}
				else if(column.DataType == typeof(int) | column.DataType == typeof(float) | column.DataType == typeof(decimal) | column.DataType == typeof(long))
				{
					part = new NumberFilterPart(textBox, column.DataType, column.ColumnName, column.ColumnName, caption, formGenerator);
				}
			}
			else if(control is AsDateBox)
			{
				part = new DateFilterPart(control as AsDateBox, column.DataType, column.ColumnName, column.ColumnName, caption, formGenerator);
			}
			else if(control is BlobControl)
			{
				part = new StringFilterPart(tb, typeof(string), column.ColumnName, column.ColumnName, caption, formGenerator);
			}
			else if(control is AsCheckBox)
			{
				part = new BoolFilterPart(control as AsCheckBox, column.DataType, column.ColumnName, column.ColumnName, caption, formGenerator);
			}
			else
			{
				part = new DummyFilterPart(control, column.DataType, column.ColumnName, column.ColumnName, caption, formGenerator);
			}
