...
public class AsTextBoxStyleColumn : DataGridTextBoxColumn
{
    ...
    )
    {
        if (cellIsVisible)
        {
            ...
            int height = (
                bounds.Height < AsTextBox.PreferredHeight
    ...
    {
        RuleEngine ruleEngine = (this.DataGridTableStyle.DataGrid.FindForm() as AsForm)
            .FormGenerator
            .FormRuleEngine;
        return ruleEngine;
    }
    ...
    private void AsTextBox_KeyPress(object sender, KeyPressEventArgs e)
    {
        ...
        {
            canEdit =
                (((e.KeyChar != ' ') || ((Control.ModifierKeys & Keys.Shift) != Keys.Shift)))
                && (
                    ((Control.ModifierKeys & Keys.Control) != Keys.Control)
