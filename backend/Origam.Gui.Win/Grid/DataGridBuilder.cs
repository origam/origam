...
public class DataGridBuilder : IGridBuilder
{
	...
	public AsDataGrid CreateGrid(object dataSource, string dataMember, Control control, Guid panelId, RuleEngine ruleEngine)
	{
		...
		grid.TableStyles.Add(SetUpGridStyle(grid, control, dataSource as DataSet, dataMember, ruleEngine));
                ...
                try
                {
                    ...
                    if (dataSource != null && ! this._form.FormGenerator.IgnoreDataChanges)
                    {
                        try
                        {
                            ...
                            if (layout != null)
                            {
                                bool dirty = (bool)Reflector.GetValue(layout.GetType(), layout, "dirty");
                                if (!dirty)
                                {
	...
	private DataGridTableStyle SetUpGridStyle (DataGrid grid, Control control, DataSet ds, string member, RuleEngine ruleEngine)
	{
		//cretate new style
	...
	private void UpdateColumns(DataGrid grid, DataGridTableStyle tableStyle, 
        bool firstTime, DataSet ds)
	{
		...
		foreach(DataGridColumnStyleHolder style in _styles)
		{
			if(firstTime)
			{
				if(! tableStyle.GridColumnStyles.Contains(style.Style.MappingName))
				{
					...
					Guid columnId = (Guid)ds.Tables[tableStyle.MappingName].Columns[style.Style.MappingName].ExtendedProperties["Id"];
	...
	private List<DataGridColumnStyleHolder> GetColumnStylesFromControls (Control control, int offset, OrigamPanelColumnConfig userConfig, RuleEngine ruleEngine)
	{
		...
		{ 
			if(item is IAsControl)
			{
				...
				if(colStyle != null)
				{
					...
					if(_useUserConfig)
					{
						...
						configRow = userConfig.PanelColumnConfig.Select("ColumnName = '" + colStyle.MappingName + "'");
						if(configRow != null && configRow.Length > 0)
						{
							OrigamPanelColumnConfig.PanelColumnConfigRow conf = configRow[0] as OrigamPanelColumnConfig.PanelColumnConfigRow;
			...
			{
				styles.AddRange(GetColumnStylesFromControls(item, offset + item.TabIndex, userConfig, ruleEngine));
			}
		}
		return styles;
	}
	private DataGridColumnStyle CreateColumnStyle(Control control,
	...
	private void SetColumnWidth(Control control, OrigamPanelColumnConfig userConfig,
		DataGridColumnStyle columnStyle)
	{
		DataRow[] configRow = null;
		if (_useUserConfig)
		{
			configRow =
				userConfig.PanelColumnConfig.Select(
		...
		if (configRow != null && configRow.Length > 0)
		{
			...
			columnStyle.Width =
				...
				.ColumnWidth;
		} else if ((control is IAsCaptionControl) &&
	...
	private DataGridColumnStyle MakeDataGridColumnStyle(Control control,
		RuleEngine ruleEngine, DataColumn column)
	{
		DataGridColumnStyle result;
		if (control is AsDropDown dropDown)
		{
			var dataGridDropdownColumn = new DataGridDropdownColumn(
				dropDown, column.ColumnName, ruleEngine);
		...
		} else
		{
			...
			if (control is AsTextBox asTextBox)
			{
				...
				if (asTextBoxStyleColumn.AsTextBox.DataType == typeof(string) |
				    asTextBoxStyleColumn.AsTextBox.DataType == typeof(object))
				{
					asTextBoxStyleColumn.TextBox.TextAlign =
						HorizontalAlignment.Left;
					asTextBoxStyleColumn.AsTextBox.TextAlign =
						HorizontalAlignment.Left;
				} else
				{
					asTextBoxStyleColumn.TextBox.TextAlign =
					...
					asTextBoxStyleColumn.AsTextBox.TextAlign =
						HorizontalAlignment.Right;
					if (!string.IsNullOrEmpty(asTextBoxStyleColumn.AsTextBox
						.CustomFormat))
					{
						asTextBoxStyleColumn.Format =
							asTextBoxStyleColumn.AsTextBox.CustomFormat;
					} else if (asTextBoxStyleColumn.AsTextBox.DataType ==
					           typeof(int))
					{
				...
				asTextBoxStyleColumn.Alignment =
					asTextBoxStyleColumn.AsTextBox.TextAlign;
	...
	private void Style_WidthChanged(object sender, EventArgs e)
	{
		if(sender is DataGridColumnStyle columnStyle)
		{
			...
			{
				this.FilterFactory.PlotControls(columnStyle.DataGridTableStyle,
	...
	{
		OrigamPanelColumnConfigDA.PersistColumnConfig(_panelId, columnStyle.MappingName, position, columnStyle.Width, hidden);
	}
	private void DataGridBuilder_RowHeaderWidthChanged(object sender, EventArgs e)
	{
		this.FilterFactory.PlotControls( sender as DataGridTableStyle, ((sender as DataGridTableStyle).DataGrid as AsDataGrid).HorizontalScrollPosition);
	}
	private void grid_SizeChanged(object sender, EventArgs e)
	{
		this.FilterFactory.PlotControls( (sender as DataGrid).TableStyles[0], (sender as AsDataGrid).HorizontalScrollPosition);
	}
	...
	private void grid_MouseDown(object sender, MouseEventArgs e)
	{
		...
		if(hti.Type == DataGrid.HitTestType.ColumnHeader)
		{
			...
			if(grid.CurrencyManager.Position >= 0 && (grid.CurrencyManager.Current as DataRowView).IsNew)
			{
				return;
			}
			string sortColumnName = this.grid.TableStyles[0].GridColumnStyles[hti.Column].MappingName;
			...
			if(e.Button == MouseButtons.Left)
			{
				if(panel != null)
				{
					if(Control.ModifierKeys == Keys.Shift)
					{
						...
						{
							panel.AddSort(sortColumnName, Schema.EntityModel.DataStructureColumnSortDirection.Ascending);
						}
					}
					else
					{
						if(panel.IsColumnSorted(sortColumnName))
						{
							Schema.EntityModel.DataStructureColumnSortDirection dir = Schema.EntityModel.DataStructureColumnSortDirection.Ascending;
							if(panel.ColumnSortDirection(sortColumnName) == Schema.EntityModel.DataStructureColumnSortDirection.Ascending)
							{
								dir = Schema.EntityModel.DataStructureColumnSortDirection.Descending;
							}
							panel.Sort(sortColumnName, dir);
						}
						else
						{
							panel.Sort(sortColumnName, Schema.EntityModel.DataStructureColumnSortDirection.Ascending);
						}
					}
				}
			}
		...
		if(hti.Column >= 0 && hti.Row >= 0 && hti.Row < grid.BindingContext[grid.DataSource, grid.DataMember].Count)
		{
			try
			{
				// If the user clicked on the checkbox column, we change the value of this column.
				if(grid.TableStyles[0].GridColumnStyles[hti.Column] is AsCheckStyleColumn && 
