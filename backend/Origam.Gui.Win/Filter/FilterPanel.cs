...
public class FilterPanel : System.Windows.Forms.UserControl
{
	...
	public void ApplyFilter(OrigamPanelFilter.PanelFilterRow filter)
	{
			...
			else
			{
				...
				{
					_ignoreQueryChange = true;
					foreach(OrigamPanelFilter.PanelFilterDetailRow detail in filter.GetPanelFilterDetailRows())
					{
						if(_filterParts.ContainsKey(detail.ColumnName))
						{
							...
							part.Value1 = OrigamPanelFilterDA.StoredFilterValue(detail, part.DataType, 1);
							part.Value2 = OrigamPanelFilterDA.StoredFilterValue(detail, part.DataType, 2);
							part.LoadValues();
						}
					}
				}
	...
	public void AddFilterDetails(OrigamPanelFilter filterDS, OrigamPanelFilter.PanelFilterRow filter, Guid profileId)
	{
			foreach(FilterPart part in _filterParts.Values)
			{
				if(part.Query != null)
				{
					OrigamPanelFilterDA.AddPanelFilterDetailRow(filterDS, profileId, filter.Id, part.GridColumnName, (int)part.Operator, part.Value1, part.Value2);
				}
			}
		}
	#endregion
