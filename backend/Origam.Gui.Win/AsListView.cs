...
public class AsListView : System.Windows.Forms.ListView, IAsDataConsumer
{
    ...
    public object SelectedValue
    {
        ...
        set
        {
            ...
            if (
                ...
                && ((IBindingList)m_currencyManager.List).SupportsSearching
            )
            {
    ...
    private void SetDataBinding()
    {
        ...
        if (BindingContext != null)
        {
            ...
            if (currencyManager != m_currencyManager)
            {
                ...
                {
                    m_currencyManager.MetaDataChanged -= new EventHandler(
                        currencyManager_MetaDataChanged
                    );
                    m_currencyManager.PositionChanged -= new EventHandler(
                        currencyManager_PositionChanged
                    );
                    m_currencyManager.ItemChanged -= new ItemChangedEventHandler(
                ...
                if (currencyManager != null)
                {
                    reloadMetaData = true;
                    reloadItems = true;
                    currencyManager.MetaDataChanged += new EventHandler(
                        currencyManager_MetaDataChanged
                    );
                    currencyManager.PositionChanged += new EventHandler(
                        currencyManager_PositionChanged
                    );
                    currencyManager.ItemChanged += new ItemChangedEventHandler(
                        currencyManager_ItemChanged
                    );
                }
            }
            if (bindingList != m_bindingList)
            {
                ...
                {
                    m_bindingList.ListChanged -= new ListChangedEventHandler(
                        bindingList_ListChanged
                    );
                }
    ...
    private void LoadColumnsFromSource(bool setDefaultWidth)
    {
        ...
        try
        {
            ...
            PropertyDescriptor[] arr = new PropertyDescriptor[
                (ValueMemberDisplayed | ValueMember == null ? 0 : 1) + columnList.Length
            ];
