...
namespace Origam.OrigamEngine.ModelXmlBuilders
{
	...
	public class AsPanelBuilder
	{
			...
			Guid formId, bool isIndependent)
		{
			...
			if(renderData.IsGridVisible) defaultPanelView = OrigamPanelViewMode.Grid;
			if(renderData.IsCalendarVisible) defaultPanelView = OrigamPanelViewMode.Calendar;
			if(renderData.IsPipelineVisible) defaultPanelView = OrigamPanelViewMode.Pipeline;
			if(renderData.IsMapVisible) defaultPanelView = OrigamPanelViewMode.Map;
			if(renderData.IsVisualEditorVisible) defaultPanelView = OrigamPanelViewMode.VisualEditor;
			parentNode.SetAttribute("DefaultPanelView", XmlConvert.ToString((int)defaultPanelView));

			if(renderData.NewRecordInDetailView)
			{
				parentNode.SetAttribute("NewRecordView", XmlConvert.ToString((int)OrigamPanelViewMode.Form));
			}
			...
			{
				parentNode.SetAttribute("MaxDynamicGridHeight", XmlConvert.ToString(renderData.MaxDynamicGridHeight));
			}

			parentNode.SetAttribute("IsDraggingEnabled", XmlConvert.ToString(renderData.IsDraggingEnabled));
			...
			if(isIndependent)
			{
				parentNode.SetAttribute("IndependentDataSourceId", XmlConvert.ToString(renderData.IndependentDataSourceId));
				parentNode.SetAttribute("IndependentDataSourceFilterId", XmlConvert.ToString(renderData.IndependentDataSourceFilterId));
				parentNode.SetAttribute("IndependentDataSourceSortId", XmlConvert.ToString(renderData.IndependentDataSourceSortId));
			}
			...
			if(renderData.IsCalendarSupported)
			{
				...
				parentNode.SetAttribute("CalendarDescriptionMember", renderData.CalendarDescriptionMember);
				parentNode.SetAttribute("CalendarIsFinishedMember", renderData.CalendarIsFinishedMember);
				parentNode.SetAttribute("CalendarNameMember", renderData.CalendarNameMember);
				parentNode.SetAttribute("CalendarResourceIdMember", renderData.CalendarResourceIdMember);
				parentNode.SetAttribute("IsCalendarSupported", XmlConvert.ToString(renderData.IsCalendarSupported));
				parentNode.SetAttribute("DefaultCalendarView", XmlConvert.ToString(renderData.DefaultCalendarView));
				parentNode.SetAttribute("CalendarResourceNameLookupField", renderData.CalendarResourceNameLookupField);
				parentNode.SetAttribute("CalendarCustomSortMember", renderData.CalendarCustomSortMember);
				if(renderData.CalendarRowHeightConstantId != Guid.Empty)
				{
					...
					string value = (string)parameterService.GetParameterValue(renderData.CalendarRowHeightConstantId, OrigamDataType.String);
					parentNode.SetAttribute("CalendarRowHeight", value);
				}
				parentNode.SetAttribute("CalendarShowAllResources", XmlConvert.ToString(renderData.CalendarShowAllResources));
				if(renderData.CalendarShowAllResources)
				{
					XmlElement calendarResourceListElement = parentNode.OwnerDocument.CreateElement("CalendarResourceList");
					parentNode.AppendChild(calendarResourceListElement);

					Guid lookupId = (Guid)table.Columns[renderData.CalendarResourceIdMember].ExtendedProperties[Const.DefaultLookupIdAttribute];
					ComboBoxBuilder.BuildCommonDropdown(calendarResourceListElement, lookupId, renderData.CalendarResourceIdMember, table);
				}
                if (renderData.CalendarViewStyle != null)
                {
                    parentNode.SetAttribute("CalendarViewStyle", renderData.CalendarViewStyle.StyleDefinition());
                }
            }
			...
			{
				parentNode.SetAttribute("IsPipelineSupported", XmlConvert.ToString(renderData.IsPipelineSupported));
				...
				parentNode.SetAttribute("PipelineStateLoookup", XmlConvert.ToString(renderData.PipelineStateLoookup));
			}

			// map view attributes
			if(renderData.IsMapSupported)
			{
				parentNode.SetAttribute("IsMapSupported",  XmlConvert.ToString(renderData.IsMapSupported));
				...
				{
					throw new ArgumentOutOfRangeException("OrigamMap_InitialZoom", zoom, "Initial zoom has to be greater than zero");
				}
			...
			{
				parentNode.SetAttribute("IsVisualEditorSupported", XmlConvert.ToString(renderData.IsVisualEditorSupported));
			}

			string entityName = FormXmlBuilder.AddDataSource(dataSources, table, controlId, isIndependent);
			...
			if(charts.Count > 0)
			{
				...
				{
					chartsElement.AppendChild(ChartBuilder.Build(chartsElement.OwnerDocument, chart, table));
				}
			}
        }
