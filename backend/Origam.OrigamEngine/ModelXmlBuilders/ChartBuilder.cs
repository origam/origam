...
public class ChartBuilder
{
	...
	private static void BuildCartesianChart(XmlDocument doc, XmlElement chartElement, CartesianChart chart, DataTable table)
	{
		...
		foreach(ISchemaItem item in chart.ChildItems)
		{
			...
			{
				BuildCartesianChartHorizontalAxis(axesElement, item as CartesianChartHorizontalAxis, table);
			}
	...
	private static void BuildSvgChart(XmlDocument doc, XmlElement chartElement, SvgChart chart, DataTable table)
	{
		...
		if(titleLookupId != null) chartElement.SetAttribute("TitleFieldLookupId", titleLookupId);
	...
	private static string GetLookupId (DataTable table, string fieldName, ISchemaItem item)
	{
		if(fieldName != "" && fieldName != null)
		{
			...
			{
				throw new ArgumentOutOfRangeException("fieldName", fieldName, "Field not found in the data source for " + item.Path);
			}
	...
	private static void BuildCartesianChartVerticalAxis(XmlElement axesElement, CartesianChartVerticalAxis item)
	{
		...
		axis.SetAttribute("Id", item.Name);
		axis.SetAttribute("Caption", item.Caption);
		if(item.ApplyMinLimit) axis.SetAttribute("Min", item.Min.ToString());
	...
	private static XmlElement BuildBasicSeries(XmlElement seriesElement, AbstractSeries item, DataTable table)
	{
		...
		string fieldLookupId = GetLookupId(table, item.Field, item);
		if(fieldLookupId != null) series.SetAttribute("FieldLookupId", fieldLookupId);
		series.SetAttribute("Aggregation", item.Aggregation.ToString());
		if(item.ColorsLookup != null) series.SetAttribute("ColorsLookupId", item.ColorsLookupId.ToString());
		return series;
	}
