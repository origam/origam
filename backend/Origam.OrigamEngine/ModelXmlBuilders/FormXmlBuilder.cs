...
public class FormXmlBuilder
{
    ...
    public static XmlDocument GetXml(WorkQueueClass wqc, DataSet dataset, string name, Guid queueId)
    {
        ...
        DataSet commands = core.DataService.Instance.LoadData(
        ...
        foreach (DataRow cmdRow in commands.Tables["WorkQueueCommand"].Rows)
        {
            if (
                cmdRow.IsNull("Roles")
                || !auth.Authorize(SecurityManager.CurrentPrincipal, (string)cmdRow["Roles"])
            )
            {
                continue;
            }
        ...
        XmlDocument doc = GetWindowBaseXml(
            name,
            Guid.Empty,
            settings.WorkQueueListRefreshPeriod,
            false,
            false,
            false
        );
        ...
        // Panel controls
        int lastPos = 5;
        var mappedColumns = wqc.ChildItemsByType<WorkQueueClassEntityMapping>(
        ...
        IOrderedEnumerable<DataRow> orderedCommands = commandRows.OrderBy(x => x["SortOrder"]);
    ...
    )
    {
        ...
        XmlNodeList grids = doc.SelectNodes(
