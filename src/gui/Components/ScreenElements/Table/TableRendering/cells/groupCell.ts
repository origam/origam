...
export function drawGroupCellBackground() {
  const ctx2d = context2d();
  ctx2d.fillStyle =  getComputedStyle(document.documentElement).getPropertyValue('--color6');
  ctx2d.fillRect(
...
function drawSelectedRowBorder() {
  const ctx2d = context2d();
  ctx2d.beginPath();
  ctx2d.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--blue');
