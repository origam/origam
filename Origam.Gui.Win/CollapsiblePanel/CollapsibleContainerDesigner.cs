	...
	internal class CollapsibleContainerDesigner : ParentControlDesigner
	{
		...
		private void OnAdd(object sender, EventArgs eevent)
		{
			...
			if (service != null)
			{
				...
				try
				{
					try
					{
						transaction = service.CreateTransaction("OnAddCollapsiblePanel: " + base.Component.Site.Name);
					...
					CollapsiblePanel page = (CollapsiblePanel) service.CreateComponent(typeof(CollapsiblePanel));
					string str = null;
			...
			if ((component != null) && (component.TabPages.Count != 0))
			{
				...
				if (service != null)
				{
					...
					try
					{
						try
						{
							transaction = service.CreateTransaction("CollapsibleContainerRemovePanel: " + selectedTab.Site.Name + ", " + base.Component.Site.Name);
							base.RaiseComponentChanging(member);
						}
						catch (CheckoutException exception)
						{
		...
		protected override void PreFilterProperties(IDictionary properties)
		{
			...
			for (int i = 0; i < strArray.Length; i++)
			{
				...
				{
					properties[strArray[i]] = TypeDescriptor.CreateProperty(typeof(CollapsibleContainerDesigner), oldPropertyDescriptor, attributes);
				}
			}
		}
