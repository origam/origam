...
test.describe("Model tree move to dialog (real backend)", () => {
  ...
  test("Move to moves a constant into a group of another package", async ({
    page,
  }) => {
    ...
    await expectMoveRequest(page, () =>
      page.getByTestId("move-to-button-move").click(),
    );
  ...
  test("Move to copies a constant into a group of another package", async ({
    page,
  }) => {
    await openConstants(page);
    await openMoveToDialog(page, "UngroupedConstant");
    ...
    await selectOption(page, "move-to-target", FOREIGN_GROUP);
    await expectMoveRequest(page, () =>
      page.getByTestId("move-to-button-copy").click(),
    );
  ...
  test("Move to is disabled for a group", async ({ page }) => {
    ...
    await expect(
      menuItem(page, "AutomaticTestsGroup", "tree-menu-move-to"),
  ...
  test("The target list is filtered while typing", async ({ page }) => {
    ...
    await expect(
      page.locator("body > ul > li", { hasText: "No matches" }),
    ).toBeVisible();
  });
...
async function openMoveToDialog(page: Page, nodeText: string): Promise<void> {
  ...
  await expectMoveTargets(page, () =>
    menuItem(page, nodeText, "tree-menu-move-to").click(),
  );
