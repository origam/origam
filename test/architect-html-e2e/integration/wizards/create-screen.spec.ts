...
test.describe('Create Screen wizard (real backend)', () => {
  ...
  test('Create Screen with all fields', async ({ page }) => {
    ...
    const nameField = dialog.getByRole('textbox', { name: 'e.g. Dimension1' });
    await nameField.fill('Dimension1Screen');
    await page.getByRole('button', { name: 'Next →' }).click();

    const captionField = dialog.getByRole('textbox', { name: 'e.g. Dimension1' });
    ...
    await expect(page.locator('tbody')).toContainText('Dimension1Screen');
    await expect(page.getByRole('cell', { name: 'Data Structure' }).first()).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Screen Section' }).first()).toBeVisible();
    await expect(page.getByRole('cell', { name: 'Screen', exact: true }).first()).toBeVisible();
  });
});
