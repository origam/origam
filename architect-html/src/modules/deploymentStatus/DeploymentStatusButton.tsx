...
const DeploymentStatusButton = observer(() => {
  const rootStore = useContext(RootStoreContext);
  const run = runInFlowWithHandler(rootStore.errorDialogController);
  const packagesState = rootStore.packagesState;

  const handleOnClick = () => {
    run({ generator: rootStore.editorTabViewState.openDeploymentStatusModule() });
  };
  ...
  return (
    <Button
      type="secondary"
      title={T('Deployment Status', 'deploymentStatus_Button_label')}
      prefix={<VscPulse />}
