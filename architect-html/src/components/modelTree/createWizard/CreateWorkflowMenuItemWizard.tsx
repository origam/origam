...
interface CreateWorkflowMenuItemWizardProps {
  workflowId: string;
...
export const CreateWorkflowMenuItemWizard: React.FC<CreateWorkflowMenuItemWizardProps> = observer(
  ({ workflowId, parentNodeName, onCancel, onCreate }) => {
    ...
    const steps = [
      {
        id: 'basics',
        label: T('Basics', 'create_workflow_menu_item_step_basics_label'),
        hint: T('Caption & role', 'create_workflow_menu_item_step_basics_hint'),
      },
      {
        id: 'review',
    ...
    const submit = () => {
      ...
      run({
        generator: function* () {
          try {
            const result = (yield rootStore.architectApi.createWorkflowMenuItem({
              workflowId,
        ...
        return (
          <>
            <h2 className={S.formTitle}>
              {T('Configure menu entry', 'create_workflow_menu_item_basics_title')}
            </h2>
            <p className={S.formSubtitle}>
              {T(
                'A Sequential Workflow Reference menu item will be added under the Main Menu, pointing to this workflow.',
                'create_workflow_menu_item_basics_subtitle',
              )}
            </p>

            <div className={S.field}>
              <label className={S.fieldLabel}>
                {T('Caption', 'create_workflow_menu_item_caption_label')}{' '}
            ...
            <div className={S.field}>
              <label className={S.fieldLabel}>
                {T('Role', 'create_workflow_menu_item_role_label')}
              </label>
              <input
                className={S.input}
                value={role}
        ...
        <>
          ...
          <p className={S.formSubtitle}>
            {T(
              ...
              'create_workflow_menu_item_review_subtitle',
            )}
          </p>
            ...
            <div className={S.reviewCardHeader}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--background6)' }}>
                  {T('Sequential Workflow Reference', 'create_workflow_menu_item_review_type')}
                </div>
              </div>
            </div>
            <div className={S.reviewKv}>
              <div className={S.reviewKey}>{T('Role', 'create_workflow_menu_item_role_label')}</div>
              <div>{role.trim() || '*'}</div>
            </div>
          </div>
        </>
        ...
        <div className={S.header}>
          ...
          <div className={S.headerText}>
            <div className={S.headerTitle}>
              {T('Create Menu from Sequential Workflow', 'create_workflow_menu_item_header_title')}
            </div>
            <div className={S.headerSubtitle}>
              {T('for {0}', 'create_workflow_menu_item_header_subtitle', parentNodeName)}
            </div>
          </div>
                ...
                {step < steps.length - 1 ? (
                  ...
                  >
                    {submitting
                      ...
                      : T('Create Menu Item', 'create_workflow_menu_item_btn_create')}
                  </button>
                )}
              </div>
            </div>
