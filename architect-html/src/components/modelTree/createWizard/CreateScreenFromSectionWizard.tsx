...
import { ICreateWizardResult, IScreenFromSectionWizardData } from '@api/IArchitectApi';
import { runInFlowWithHandler } from '@errors/runInFlowWithHandler';

interface CreateScreenFromSectionWizardProps {
...
export interface ScreenFromSectionModel {
  name: string;
}

export const CreateScreenFromSectionWizard: React.FC<CreateScreenFromSectionWizardProps> = observer(
  ({ screenSectionId, parentNodeName, onCancel, onCreate }) => {
    ...
    const [sectionData, setSectionData] = useState<IScreenFromSectionWizardData | null>(null);
    const [loading, setLoading] = useState(true);
    const [submitting, setSubmitting] = useState(false);
    const [model, setModel] = useState<ScreenFromSectionModel>({ name: '' });

    const steps = [
      {
        id: 'screen',
        label: T('Screen', 'create_screen_from_section_step_screen_label'),
        hint: T('Name the screen', 'create_screen_from_section_step_screen_hint'),
      },
      {
        id: 'review',
    ...
    useEffect(() => {
      ...
      run({
        generator: function* () {
          try {
            const data = (yield rootStore.architectApi.getScreenFromSectionWizardData(
              screenSectionId,
            )) as IScreenFromSectionWizardData;
            if (cancelled) return;
            setSectionData(data);
            setModel(prev => ({ ...prev, name: prev.name || data.sectionName || '' }));
          } finally {
    ...
    }, [screenSectionId, run, rootStore.architectApi]);
    ...
    const nameExists = useMemo(
      () =>
        (sectionData?.existingDataStructureNames ?? []).some(
          existingName => existingName.toLowerCase() === trimmedName.toLowerCase(),
        ),
      [sectionData?.existingDataStructureNames, trimmedName],
    );
    ...
    const submit = () => {
      ...
      run({
        generator: function* () {
          try {
            const result = (yield rootStore.architectApi.createScreenFromSection({
              screenSectionId,
              name: trimmedName,
    ...
    const renderForm = () => (
      <>
        <h2 className={S.formTitle}>
          {T('Name the screen', 'create_screen_from_section_screen_title')}
        </h2>
        <p className={S.formSubtitle}>
          {T(
            'This wizard creates a Data Structure and a Screen from this screen section. Choose a unique name.',
            'create_screen_from_section_screen_subtitle',
          )}
        </p>

        <div className={S.field}>
          <label className={S.fieldLabel}>
            {T('Name', 'create_screen_from_section_name_label')}{' '}
          ...
          {nameExists && (
            <span className={S.fieldError}>
              {T(
                'Name of Structure already exists.',
                'create_screen_from_section_name_exists_error',
              )}
            </span>
          )}
        </div>
    ...
    const renderReview = () => {
      ...
      return (
        <>
          ...
          <p className={S.formSubtitle}>
            {T(
              'This wizard will create a Screen from a Screen Section with these parameters:',
              'create_screen_from_section_review_subtitle',
            )}
          </p>
          ...
          <div className={S.reviewCard}>
            <div className={S.reviewKv}>
              <div className={S.reviewKey}>
                {T('Data Structure', 'create_screen_from_section_review_data_structure')}
              </div>
              <div>{trimmedName}</div>
            </div>
            <div className={S.reviewKv}>
              <div className={S.reviewKey}>
                {T('Screen', 'create_screen_from_section_review_screen')}
              </div>
              <div>{trimmedName}</div>
            </div>
          </div>
        ...
        <div className={S.header}>
          ...
          <div className={S.headerText}>
            <div className={S.headerTitle}>
              {T('Create Screen', 'create_screen_from_section_header_title')}
            </div>
            <div className={S.headerSubtitle}>
              {T('in {0}', 'create_screen_from_section_header_subtitle', parentNodeName)}
            </div>
          </div>
                ...
                {step < steps.length - 1 ? (
                  ...
                  >
                    {submitting
                      ...
                      : T('Create Screen', 'create_screen_from_section_btn_create')}
                  </button>
                )}
              </div>
            </div>
